...
<Frame>
  ![CoachIQ client Transactions tab with revenue summary and payment history](/images/client-transactions.png)
</Frame>
...
| **Update payment method** | Client does this in their Billing settings; you can see cards on file and the default on the **Payment Methods** tab ([details](/people/client-profiles/client-profiles-subscriptions#payment-methods-on-file)) |
...
## Purchases paid by a parent

A managed athlete's transaction totals include purchases paid for them by a parent. The parent's **Filter by products** options also include products purchased for a managed athlete. Selecting a product narrows the list; clearing it restores the full list without dropping parent-paid purchases from the totals.
