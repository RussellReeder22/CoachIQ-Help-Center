...
icon: "rotate"
keywords: ["subscriptions", "membership status", "recurring billing", "active subscription", "canceled subscription", "monthly revenue", "cards on file", "default card", "payment methods"]
---
...
<Frame>
  ![CoachIQ client Subscriptions tab with status filters and monthly revenue](/images/client-subscriptions.png)
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Manually canceling their subscription">
    **You can't do it from this tab.** To cancel:
    ...
    [Learn how to cancel subscriptions →](/payments/manage-subscriptions)
  </Accordion>
</AccordionGroup>
...
Next to Subscriptions, the profile's **Payment Methods** tab lists every card the client has saved with you and marks which one is the **default**. The default is the card Stripe charges on the next renewal.

**Use it to:**

- Confirm a new card landed after the client updated it in their portal (**Billing**). Check the **default** label separately from whether the new card is listed. Adding a card through the admin flow preserves the existing default.
...
### Add a card for a client

1. Open **People → Clients**, select the client, and open **Payment Methods**.
2. Choose **Add card** and enter the card details with the client's permission.
3. Confirm the consent to save the card for future payments and save it.
4. Check that the card appears in the list. If it should become the default, choose **Set as default** separately.

Saving a card does not charge it and preserves the existing default. Cancelling the form leaves saved cards unchanged. If the card is declined, correct the details or use another card; existing saved cards stay in place.

The tab reads **No payment methods on file** when no cards have been saved. Clients can also manage cards from **Billing** in the athlete portal, or save one through a [card-on-file product](/payments/product-builder#section-2-payment-options). See [Updating client payment methods](/payments/manage-subscriptions#updating-client-payment-methods).
...
| **See cards on file / default card** | Payment Methods tab              |
