...
icon: "box"
keywords: ["products", "purchase history", "what did they buy", "add product", "assign product", "client purchases", "configure purchases", "cash", "check", "offline payment", "stripe subscription id"]
---
...
<Frame>
  ![CoachIQ client Products tab showing purchased items and product types](/images/client-products.png)
</Frame>
...
Give a client a product they didn't buy through checkout, and record how (or whether) you were paid:
...
<Steps>
  ...
  <Step title="Select product">
    Click the product you want to grant them. The **Configure Purchases** step opens.
  </Step>
  <Step title="Configure the purchase">
    **Configure Purchases** shows the product and price, then asks:

    - **Stripe Subscription ID** (optional) - for a subscription product, paste the athlete's existing Stripe subscription ID to link the purchase to real billing, so renewals and credits follow Stripe. Leave blank for a complimentary grant.
    - **Credits** - **Award now** or **Award upon cycle renew**, for credit-bearing products.
    - **How the purchase was collected** - record offline money: cash, check, or other, and whether it is paid or still being collected. Use this when a family paid you in person or by check, so the roster and the client's entitlements match reality without the payment going through Stripe.

    The collection details are saved with the purchase for your records. They don't create a Stripe charge or a Transactions entry.
  </Step>
  <Step title="Confirm">
...
<Frame>
  ![Configure Purchases step with the optional Stripe Subscription ID field](/images/athlete-profile-add-product-subscription-id.png)
</Frame>

<Note>
  **No card is charged** when you manually add products. CoachIQ never initiates a charge from this screen; it either links an existing Stripe subscription or records what you collected yourself. Use this for:

  - Complimentary memberships
  - Staff/family member access
  - Makeup sessions after issues
  - Trial period access
  - Cash, check, or bank-transfer payments taken outside CoachIQ
</Note>

## Archiving a client's product

When a membership has ended or the wrong product was added, **Archive** the product from the client's Products tab. Archiving:

- Removes the product's remaining credits, so the athlete can no longer book on it
- Keeps the purchase history on the profile, so past sessions and payments still read correctly
- Leaves your catalog product and every other client untouched

A confirmation spells out the credit and booking-access change before anything happens. If the product is an active subscription, set the subscription to end first (see [Managing subscriptions](/payments/manage-subscriptions)); a product that is still billing cannot be archived.

<Note>
  Archive replaces the old dead end where a credit-bearing product with used credits could not be removed and the profile said to contact Support. If you only want to stop the credits and keep the product showing as active, use the credit toggle instead; see [Turning off a product's credits](/people/client-profiles/client-profiles-credits#turning-off-a-products-credits).
</Note>
...
| **Turn off a product's credits** | Credits tab → toggle on the product's credit tab |
| **Retire a product and its credits** | This tab → **Archive** on the product (see above) |
| **See cards on file**    | Payment Methods tab                  |
