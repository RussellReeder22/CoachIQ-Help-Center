...
icon: "calendar-check"
keywords: ["bookings", "session history", "past sessions", "upcoming sessions", "attendance", "canceled bookings", "client bookings", "parent bookings", "grouped by athlete"]
---
...
<Frame>
  ![CoachIQ client Bookings tab showing upcoming, canceled, and past sessions](/images/client-bookings.png)
</Frame>
...
## Parent profiles: bookings grouped by athlete

Open a **Parent** and the Bookings tab shows every session for every athlete connected to that parent, grouped by athlete, rather than only sessions the parent booked for themselves. Open one of the athletes and you see just that athlete's sessions.

This mirrors what the parent sees in their own portal and in the native app, so when a parent calls about "Tuesday's practice" you are both looking at the same list. If a parent's Bookings tab is missing a child's sessions, check **PROFILE** → **Family** on the parent; the athlete must be connected there. See [Family accounts](/athlete-portal/family-accounts#how-a-parent-sees-their-athletes-bookings).
...
- Booked through a different account. A parent's Bookings tab lists connected athletes' sessions grouped by athlete, so if the parent is empty too, check **PROFILE** → **Family** to confirm the athlete is connected
