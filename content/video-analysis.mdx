...
icon: "video"
keywords: ["video analysis", "video feedback", "annotate video", "video breakdown", "technique analysis", "drawing tools", "coaching video", "film review"]
---
...
Finish recording and save the resulting analysis before leaving the editor. Do not rely on an unfinished Create Video draft surviving navigation away.

<Warning>

**Before leaving**: Save the completed recording and confirm that it appears in Media Hub. Unfinished editor work may be lost when you navigate away.
...
Re-recording replaces the video inside the existing Media Hub item. The item and shared link stay the same, and **Send to users** and **Copy Link** remain available. A Processing label can appear while the replacement is prepared; wait for playback to become available. If saving fails after losing your connection, reconnect and use **Retry Save**.

Changing the aspect ratio when slides or drawings are prepared asks for confirmation. **No** preserves the work; **Yes** resets it to a blank slide. Do not use an aspect-ratio change as a way to resize prepared work without reviewing that warning.
