...
**Initial fees are discounted too.** If the membership has an [initial down payment](/payments/product-builder#section-2-payment-options), the coupon reduces that up-front fee as well as the recurring charge. A 100%-off code on a month-to-month membership therefore makes the first payment genuinely free, rather than free on the recurring price only.
