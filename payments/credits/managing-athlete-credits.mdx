...
keywords: ["manage credits", "add credits", "grant credits", "credit balance", "credit log", "remove credits", "master credits", "deactivate credits", "configure purchases", "cash", "check"]
---
...
<Note>
  **Prerequisites**:
  ...
  - Credit-issuing products already created ([Setting Up Credit-Based Sessions](/scheduling/setting-up-payment-options#option-3-setting-up-credit-based-sessions))
  - Understanding of credit types ([Credit System Overview](/payments/credits/credit-system-overview))
...
<Tabs>
  <Tab title="Master Credits Tab">
    ...
    **Example display:**
    ...
    <Frame>
      ![CoachIQ master credits tab showing universal credit balance for an athlete](/images/master-credits-ui.png)
    </Frame>
  ...
  <Tab title="Product-Specific Tabs">
    ...
    **Example display:**
    ...
    <Frame>
      ![CoachIQ product-specific credits tab showing redeemable schedulers and usage](/images/product-specific-credits-ui.png)
    </Frame>
    **Key indicators:**
...
<Steps>
  ...
  <Step title="Open Master Credits tab">
    ...
    <Frame>
      ![CoachIQ master credits tab with editable credit balance field](/images/master-credits-ui.png)
    </Frame>
  </Step>
...
<Steps>
  ...
  <Step title="Manually add product to athlete (if needed)">
    ...
    1. Open the **Products** tab and click **\+ Add Product**
    2. Search for and select the product
    3. In **Configure Purchases**, fill in what applies:
       - **Stripe Subscription ID** (optional) to link a subscription product to an existing Stripe subscription so renewals drive the credits
       - **Credits**: **Award now** or **Award upon cycle renew**
       - How the purchase was collected, if you took payment outside CoachIQ: cash, check, or other, and whether it is paid or still being collected

    <Warning>
      **Payment consideration**: Adding a product manually never charges a card. It either links an existing Stripe subscription or records a complimentary or offline (cash/check) purchase. Credits from a complimentary grant stay live until you turn them off from the product's credit tab; see [Turning off a product's credits](/people/client-profiles/client-profiles-credits#turning-off-a-products-credits).
    </Warning>
  </Step>
...
<Note>
  **Edge case**: Manually adding a product to an athlete WITHOUT charging them keeps those credits live indefinitely (until you turn the product's credits off, or remove the product while its credits are still unused). This is useful for:
...
<Tabs>
  ...
  <Tab title="Option 3: Create Promotional Product">
    ...
    **Benefits**:
    ...
    - Clean credit logs
    - Can turn the product's credits off later from each athlete's credit tab, or archive the product from their Products tab to remove remaining credits while keeping the history (see [Archiving a client's product](/people/client-profiles/client-profiles-products#archiving-a-clients-product))
...
<Steps>
  ...
  <Step title="Verify product is linked to schedulers">
    ...
    **Solution**: Grant Master Credits or add scheduler to product's redeemable list ([Setting Up Credit-Based Sessions](/scheduling/setting-up-payment-options#option-3-setting-up-credit-based-sessions))
  </Step>
...
<AccordionGroup>
  <Accordion title="Can I delete credit logs?">
    ...
    **What you CAN do**:
    ...
    - Turn the product's credits off from its credit tab (keeps the history, stops redemption)
    - Archive the product from their Products tab, which removes remaining credits and keeps the purchase history
