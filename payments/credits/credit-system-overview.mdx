...
icon: "ticket"
keywords: ["credits", "session credits", "credit packages", "credit balance", "buy credits", "use credits", "how credits work", "punch card", "prepaid sessions", "credits products", "add credits to product", "credits in products"]
---
...
<Info>
  **Where to configure credits**: Payments → Products → [Product] → Credits section. Credits are optional—many coaches use products for payment only and never issue credits.
</Info>
...
<Steps>
  ...
  <Step title="Athlete books sessions">
    ...
    <Frame>
      ![Booking payment screen showing the option to use a credit instead of paying](/images/credit-and-payment-options.png)
    </Frame>
...
<Tabs>
  <Tab title="One-Time Payment Products">
    ...
    **Configuration**: Create product → Enable "Add credits once purchased" → Set credit amount
    ...
    <Frame>
      ![CoachIQ product setting to add credits once a one-time payment is purchased](/images/add-credits-once-purchased.png)
    </Frame>
  </Tab>
  <Tab title="Subscription Products">
    ...
    **Special feature**: Can set "Unlimited credits" for subscription period (displays as `∞` symbol)
    ...
    <Frame>
      ![CoachIQ subscription product setting to issue credits per billing cycle](/images/sub-add-credits.png)
    </Frame>
  </Tab>
</Tabs>

**What you set in the Product Builder (Credits section):**

- Toggle credits ON ("Add credits once purchased")
- Number of credits issued (e.g., 10 credits)
- Expiration (30 days, 90 days, never, etc.)
- Which schedulers accept these credits (Usage Limits → Redeemable Schedulers)

<Frame>
  ![Credit Limits in Product Builder](/images/credit-limits-product.png)
</Frame>
...
<Note>
  **Important**: You create credit-issuing products in **Payments → Products**, then configure schedulers to accept those credits in **Schedule → Schedulers**. It's a two-way linkage—both sides must be configured. Step-by-step: [Setting up credit-based sessions](/scheduling/setting-up-payment-options#option-3-setting-up-credit-based-sessions).
</Note>
...
<Tabs>
  <Tab title="Master Credits (Universal)">
    ...
    **When to issue:**
    ...
    <Tip>
      Note: Master credits are only added manually by admins, under the athlete's profile → Credits tab. All other credits purchased by users will have a product specific credit bank on their account.
    </Tip>

    <Frame>
      ![Master Credits UI](/images/master-credits-ui.png)
    </Frame>
  ...
  <Tab title="Scheduler-Specific Credits">
    **What they are**: Product credits restricted to designated schedulers only

    Every product purchase creates its own **product-specific credit bank** on the athlete's account. Left unrestricted, that bank behaves like Master Credits (bookable on any credit-enabled scheduler). Restrict it to certain schedulers and you get scheduler-specific credits.
    ...
    **How they work:**
    ...
    <Frame>
      ![Product Specific Credits UI](/images/product-specific-credits-ui.png)
    </Frame>
    ...
    **Configuration**: In product builder → Usage Limits → "Redeemable Schedulers" → Select specific schedulers
...
Athletes can see their credit balance within the **Billing** section of the athlete portal

**How to navigate as an athlete:**
...
<Frame>
  ![Athlete portal billing page showing credit balance and redeemable sessions](/images/billing-credits.png)
</Frame>
...
<Frame>
  ![CoachIQ coach dashboard showing athlete credit balances and usage details](/images/credits-page.png)
</Frame>

<Note>
  **Configured vs. managed**: Credits are **configured** in Products, but day-to-day they're **managed** on the athlete's profile—viewing balances, manually adding or removing credits, tracking expirations, reviewing usage logs, and troubleshooting. See [Managing Athlete Credits](/payments/credits/managing-athlete-credits).
</Note>
...
**Product-specific format**: When credits are restricted to a scheduler, the scheduler name follows the count—`3/∞ Rising Stars` (3 used of unlimited for "Rising Stars") or `5/10 Youth Training` (5 of 10 for "Youth Training"). The same notation appears in the athlete's profile under the Credits tab and during booking flows.
...
<AccordionGroup>
  <Accordion title="5-Credit Starter Package">
    ...
    **Typical duration**: 2-3 weeks (2x/week training) or 1 month (1-2x/week training)
  ...
  <Accordion title="Unlimited Monthly Subscription">
    ...
    **Example**: "\$400/month Unlimited - 12 Credits/month (up to 3x/week)"
    ...
    See [Subscription Scheduling](/payments/credits/recurring-credit-memberships) for full details.
  </Accordion>
</AccordionGroup>
...
Three product setups that cover most coaching businesses:
...
<Tabs>
  <Tab title="Simple session package">
    ...
    **Product setup:**
    ...
    - Credits: 10
    - Expiration: 90 days
    - Usage: Any credit-enabled scheduler (no restrictions)
...
<Tabs>
  ...
  <Tab title="Day/Month After Purchase Expiration">
    **Policy**: Credits expire days or months after pruchase
...
See [Subscription Scheduling](/payments/credits/recurring-credit-memberships) for full comparison.
...
<AccordionGroup>
  <Accordion title="What if athlete books but doesn't have enough credits?">
    ...
    **Athlete sees (if credit renewal link is enabled)**: "Add more credits"
  ...
  <Accordion title="How do credits work with recurring bookings?">
    **Important**: If a client is only on recurring booking then credits are not needed. Simply collect their payment with a one time or monthly product and create their recurring booking.
...
<Tabs>
  ...
  <Tab title="Maybe skip credits">
    **Your business has these traits:**
    ...
    ❌ You offer an inconsistent training schedule (no facility, rotating gym schedule)\
    ❌ Athletes train inconsistently\
    ❌ Athletes train the same time each week (use recurring booking alongside monthly subscription—buy the membership, then you create the recurring booking as an admin)\
    ...
    **Verdict**: Start with direct payment, add credits later if needed. Credits add complexity—only use them if packages benefit your business model.
  </Tab>
...
<CardGroup cols={3}>
  <Card title="Setting Up Credit-Based Sessions" icon="box" href="/scheduling/setting-up-payment-options#option-3-setting-up-credit-based-sessions">
    Step-by-step: create credit products, connect schedulers, test the flow.
  </Card>
  <Card title="Product Builder: Credits" icon="hammer" href="/payments/product-builder#section-3-credits">
    The product side: number of credits, expiration, which schedulers accept them.
  </Card>
  <Card title="Credits on a Scheduler" icon="sliders" href="/scheduling/credits-and-schedulers">
    The scheduler side: Paywalled → How they pay: Credit, credit cost, the Buy more credits link, refunds on cancellation, edge cases.
  </Card>
  <Card title="Managing Athlete Credits" icon="user-circle" href="/payments/credits/managing-athlete-credits">
    View balances, grant credits manually, track usage, handle refunds
  </Card>
  <Card title="Credit Usage Limits" icon="lock" href="/payments/credits/credit-usage-limits-restrictions">
    Advanced restrictions and scheduling limits for credit-based sessions
  </Card>
...
<Info>
  **Ready to implement credits?** Continue to [Setting up credit-based sessions](/scheduling/setting-up-payment-options#option-3-setting-up-credit-based-sessions) for step-by-step setup instructions, or contact support for personalized credit strategy consultation. [Contact Support](https://support.coachiq.com)
</Info>
