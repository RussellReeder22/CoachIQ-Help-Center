...
icon: "rotate"
keywords: ["subscriptions dashboard", "view subscriptions", "active subscriptions", "MRR", "recurring revenue", "membership list"]
---
...
<Frame>
  ![CoachIQ Subscriptions dashboard showing active subscriptions and MRR metrics](/images/subscription-landing.png)
</Frame>
...
<Frame>
  ![CoachIQ subscriptions page product filter dropdown](/images/filter-products.png)
</Frame>

## Check for unsynced subscriptions

An **unsynced subscription** is one that exists in CoachIQ but isn't linked to Stripe. When a subscription is unsynced, the platform doesn't know to process payments, issue credits, or manage billing for that client.

Use the **Unsynced** filter to quickly identify any subscriptions that may have slipped through without a Stripe connection.
...
<Steps>
  ...
  <Step title="Click the Unsynced filter">
    ...
    <Frame>
      ![Subscriptions dashboard filtered to show unsynced subscriptions](/images/subscriptions-unsynced-filter.png)
    </Frame>
  </Step>

  <Step title="Review and resolve">
    ...
    If you find unsynced subscriptions that should be billing, contact [CoachIQ support](mailto:info@coachiq.io) to get them resynced.
  </Step>
</Steps>
...
Click Open under the Source column of the subscription to view on Stripe:
...
<Frame>
  ![Stripe subscription detail page showing billing and payment history](/images/stripe-sub-example.png)
</Frame>
...
<Steps>
  ...
  <Step title="Automatic renewals">
    ...
    New credits issued each period (if configured). Client continues accessing services.
  </Step>
...
<Tabs>
  ...
  <Tab title="Canceled subscription">
    ...
    **What to do:** If accidental: Client must resubscribe by purchasing product again. If intentional: Monitor for resubscription opportunities.
  </Tab>
  <Tab title="Trial subscription">
    **What you see:**
    ...
    - No charges yet
    - Clients have the same access as an Active subscription

    **What to do:** Nothing until trial ends. First charge processes automatically on trial end date.

    <Note>
      Trialing is currently only set manually as the coach or Admin from Stripe.
    </Note>
  </Tab>
</Tabs>
...
For advanced subscription management, use Stripe Dashboard. [Learn about Stripe Dashboard →](/payments/stripe-dashboard)
