...
The Zapier **Send an SMS Message** action is capped at 600 characters. Keep to 160 or fewer for a single segment; longer texts within the cap are sent as multiple segments and billed per segment from your [SMS credits](/inbox/sms-credits). For long text, use **Send an In-App Message** or **Send an Announcement** instead (up to 15,000 characters).

This cap is specific to the Zapier action. The **Send an SMS Message** action inside CoachIQ's own [automation builder](/tools/automations/triggers-actions-reference#all-actions) has no 160-character cap; its counter only shows where the text splits into a second segment.
