...
<Steps>
  ...
  <Step title="Rewrite the message in your voice">
    Open each action under **Actions**. Keep the dynamic fields (the `@` CoachIQ fields such as the athlete's first name) and change the rest. For SMS, watch the counter: past 160 characters the text still sends, but as two billed segments.
  </Step>
  <Step title="Test Run">
...
<AccordionGroup>
  ...
  <Accordion title="Will using a template again create a duplicate?">
    Yes, each **Use Template** creates a new automation. Archive the extras from the Automations list, or disable them from the automation's **Settings** section.
  </Accordion>
