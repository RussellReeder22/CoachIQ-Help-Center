...
<Steps>
  ...
  <Step title="Get your Group ID and API Key from CoachIQ">
    ...
    **API Key:** Click **+ New API Key**, then click **Copy** on the key's row.

    <Info>
    Your API Key is **128 characters long**. You can re-copy it at any time from its row on the API Keys page, so it is not lost if you navigate away. You can have up to 10 keys per group; delete an old one if you hit the limit. Treat each key like a password and paste it only into the Zapier connection dialog.
    </Info>
  </Step>
...
<Warning>
**Zapier may show a CoachIQ step as successful even when it failed.** CoachIQ returns "OK" at the network level and reports problems in an `error` field inside the response. If a Zap "worked" but nothing happened, open the step's Data Out and look for `error`. Every message is listed in [Zapier troubleshooting](/tools/automations/zapier-troubleshooting).
</Warning>
...
|--------|--------------|----------|-------|
| **Create User** | Adds a new athlete to your group | Email (valid format) | If that email already exists, returns the existing athlete — no duplicate is created. A genuinely **new** user receives a set-your-password email from CoachIQ immediately; there is no way to suppress it |
| **Send an In-App Message** | Sends an in-app message to one athlete | Message + email or phone | Max 15,000 characters. Optional: `From Email` — must be an admin's email in your group |
| **Send an SMS Message** | Texts one athlete | Message + email or phone | Max 600 characters; aim for 160 for single-segment delivery |
| **Send an Announcement** | Posts an announcement to one athlete or everyone | Message | Message up to 15,000 characters, heading up to 5,000. Set **Send to everyone → yes** to broadcast to the whole group |
...
<Warning>
**Zapier's Test button sends for real.** Testing Send an In-App Message, Send an SMS Message, or Send an Announcement delivers an actual message, and testing Create User creates an actual athlete (and emails them). Test with your own email or phone number, and never test an Announcement with **Send to everyone** set to yes against a real group.
</Warning>
...
3. Test — you'll see the athlete's record returned. If that email is already in your group, CoachIQ returns the existing athlete instead of creating a duplicate. That's expected behavior. If the email is new, that person gets a set-your-password email right away, so test with an address you control.
4. Turn the Zap **On**.
...
<AccordionGroup>
  ...
  <Accordion title="My API Key looks too short or the connection won't save">
    A real CoachIQ API Key is 128 characters. If Zapier rejects it for being too short, you likely pasted a partial key. Re-copy the full key using the **Copy** button on the key's row under **Settings → Developer → API Keys**. You only need a new key if the old one was deleted.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="API Keys" icon="key" href="/settings/developer#api-keys">
    Find your Group ID and generate API keys
  </Card>
  <Card title="Webhooks" icon="webhook" href="/settings/developer#creating-a-webhook">
