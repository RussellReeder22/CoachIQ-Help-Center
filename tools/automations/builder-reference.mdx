...
The **Enabled** toggle. Off means the automation is saved but never runs. There is no separate delete button; the section says "Disable above to delete this automation", so switch Enabled off first and the delete option appears. Deleting is permanent — there is no archive or restore, so leave it disabled if you may want it back.
