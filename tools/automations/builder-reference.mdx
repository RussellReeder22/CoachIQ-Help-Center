...
<Note>
  Screenshots of the builder are being refreshed. Section names and labels were checked against the live app on 2026-09-02. For the full list of triggers, actions, and dynamic fields see the [Triggers and actions reference](/tools/automations/triggers-actions-reference).
</Note>
...
Click **Add Rule** for another rule. Rules sit inside groups, and you choose how a group combines them:
...
The **Enabled** toggle. Off means the automation is saved but never runs. There is no separate delete button; the section says "Disable above to delete this automation", so switch Enabled off first and the delete option appears. Prefer **Archive** from the Automations list if you may want it back.
