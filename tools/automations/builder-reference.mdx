...
icon: "wrench"
keywords: ["automation builder", "data sources", "conditions", "and/or groups", "execution logs", "test run", "enabled toggle", "delete automation", "archive automation", "restore automation", "guardians", "dynamic fields", "@ fields"]
---
...
**Target User** has three choices on the messaging and tag actions: **User from trigger** (the person the event happened to), **trigger user and their guardians**, or a specific person you pick after clicking **Change**. Choose the guardians option when the triggering athlete may be a managed child with no email or login of their own: a welcome announcement, team tag, or practice reminder then reaches the parent as well, instead of landing on a profile nobody checks. A parent who triggers the automation is a single target; the option never adds children.

The **180 / 160** counter is a cost hint, not a limit. Texts longer than 160 characters still send; they go out as more than one segment and each segment is billed from your SMS credits (see [How SMS credits work](/inbox/sms-credits)).
