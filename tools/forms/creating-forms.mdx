...
icon: "hammer"
keywords: ["create form", "build form", "form builder", "form fields", "custom form", "form templates", "add fields", "edit form", "archive form", "restore form", "field label", "phone field"]
---
...
| Heading              | Add a section title (display only; it is not the label of the field below it) |
...
| Phone         | Phone number input; accepts numbers typed with dashes or spaces, and validates them | Secondary contact number |
...
<Steps>
  ...
  <Step title="Add fields">
    ...
    <Warning>
      **The Label is what appears in responses.** The field's **Label** is the column title in Responses and exports and the name automations see. A **Heading** placed above a field is display-only and is not connected to it. If you put the question in a Heading and example text such as "EX: John" in the Label, your responses show "EX: John" as the question. Put the question in the Label and use the default value for example text.
    </Warning>
  </Step>
  ...
  <Step title="Add headings and instructions">
    Use **Heading** fields to create sections (e.g., "Athlete Information", "Parent/Guardian Details"). Use **Plain Text** to add instructions or context. Neither is a question: headings and plain text never appear in responses, and they do not label the field beneath them.
  </Step>
...
1. Open **Tools → Forms** and find the original form.
2. Hover over its row to reveal the actions on the right. Choose **Duplicate form**, the overlapping-squares icon.
3. Open the copy, update its name for the new camp or season, and review its questions and connections before sharing.

<Frame>

![Form row with the duplicate icon followed by public-link, edit, and archive actions](/images/form-duplicate-action-2026-09.png)

</Frame>

Duplication copies the current questions, choices, and settings. The copy starts with **zero responses**; submissions stay with the original form. Review the copied questions and connections before sharing it for a new season or camp.

Editing questions does not erase historical answers. Removed questions and choices remain readable in past submissions, while questions added later are blank on those earlier submissions. See [Managing response history](/tools/forms/responses-and-automations#manage-response-history).

## Archiving and restoring a form

When a form has run its course (last season's registration, a closed camp), archive it instead of deleting it. Archiving keeps every past submission and takes the form out of circulation.

<Steps>
  <Step title="Archive it">
    Go to **Tools → Forms**, hover over the form's row, and click the **archive** icon on the right (next to the share and edit icons).
  </Step>
  <Step title="Find it later">
    Switch the **Show** filter at the top of the list from **Active** to **Archived**. Archived forms can be opened, viewed, and edited from there.
  </Step>
  <Step title="Restore it">
    Open the archived form and restore it. It moves back to the **Active** list and its links and embeds work again.
  </Step>
</Steps>

What archiving does:

- Stops the form from accepting submissions on any live page or link, so an old form cannot keep quietly collecting responses
- Keeps the form, its settings, and all of its responses
- Is reversible at any time

Full details, including what happens to shared links, are in [Sharing forms → Archiving a form](/tools/forms/sharing-forms#archiving-a-form). Deleting a form is permanent and removes its responses; export first if you must delete (see [Responses & Automations](/tools/forms/responses-and-automations#exporting-responses)).
...
<Tip>
  With Login/Signup enabled, anyone who enters an email that already has an account is asked to sign in mid-form. That is expected. See [What Returning Visitors See](/tools/forms/returning-visitor-sign-in) for exactly what they see and what to tell them.
</Tip>

<Note>
  The phone field on the Login/Signup page defaults to **+1** with a country selector, so a US number typed without a country code is saved correctly. Numbers typed with dashes are accepted.
</Note>
...
<CardGroup cols={2}>
  <Card title="Collect Payments & Bookings" icon="credit-card" href="/tools/forms/payments-and-bookings">
    Add products and schedulers to your forms
  </Card>
  <Card title="Share Your Forms" icon="share" href="/tools/forms/sharing-forms">
