...
icon: "credit-card"
keywords: ["form payment", "collect payment", "registration fee", "form booking", "book from form", "payment on form", "checkout form"]
---
...
## Require a form when someone buys a product

To start registration from a product purchase, open **Payments → Products**, edit the product, and choose **Advanced → Add a form**. Select your form and save.

The buyer completes registration, reviews their details, and then purchases. A returning parent or athlete with completed registration on file for that product skips the form on a repeat purchase. The product's recipient setting still controls whether the purchase and credits go to the buyer or a selected managed athlete.

See [Attach a registration form to a product](/payments/product-builder#attach-a-registration-form-to-a-product). Use the steps below when you want to start from the form instead.
...
## When required booking answers become a response

For a session booking that requires registration, answers are held during payment review. A declined or abandoned payment does not create a submitted response. After a successful payment, the confirmed booking and its registration response are saved. For a booking covered by credits or a membership, registration completes when the booking is confirmed.

If a payment is declined, correct the payment method and retry the same checkout. Review whether payment or booking completed before starting over.
