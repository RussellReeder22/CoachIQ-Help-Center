...
<AccordionGroup>
  ...
  <Accordion title="Credits expire after the booking is made but before the session date">
    It depends on **Sessions must occur by expiration** on the product. With it on, the session date must fall within the credit period, including when rescheduling. With it off, credits need to be valid when booked, and the session may happen later. A qualifying cancellation refund keeps the original expiration date; canceling does not extend it. See [Require sessions to happen before credits expire](/payments/product-builder#require-sessions-to-happen-before-credits-expire).
  </Accordion>
