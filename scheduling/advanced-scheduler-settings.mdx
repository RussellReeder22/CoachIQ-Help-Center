...
icon: "sliders"
keywords: ["scheduler settings", "coaches", "locations", "reminders", "booking reminders", "assign coach", "session location"]
---
...
<Note>
  **Required**: You must have a scheduler already created. This article covers settings you configure AFTER basic scheduler creation. If you haven't created a scheduler yet, see [Creating Your First Scheduler](/scheduling/creating-your-first-scheduler).
</Note>

**These settings covered in other articles**:

- Capacity, booking limits, cancellation policies → [Scheduler Settings: Limits & Permissions](/scheduling/scheduler-settings-limits-and-permissions)
- Buffer time and scheduling windows → Also covered in Limits & Permissions article
- Payment configuration → [Payment Options](/scheduling/setting-up-payment-options)
...
<Frame>
  ![Selecting multiple coaches for a scheduler](/images/multi-connected-coaches.png)
</Frame>
...
**Optional override**: In the **Connected Coaches** section there's a checkbox labeled "Check this box to allow bookings for this session even if the connected coaches are already scheduled for other sessions at the same time."

- **Unchecked** (default): System prevents double-booking ✅
- **Checked**: The connected coach can be booked for this session even when already scheduled elsewhere at the same time (NOT recommended)

<Warning>
  **Leave this box unchecked** unless you have a specific reason. Allowing the same coach to be booked in two places at once defeats the purpose of coach assignment and leads to scheduling conflicts.
</Warning>

Resources have the same override: **Allow resource double booking** in the scheduler's **Resources** section. See [Resources](/scheduling/resources#allow-resource-double-booking).

### Configuration steps

<Frame>
  ![Connected Coaches settings showing coach selection and double-booking prevention](/images/connected-coaches-ui.png)
</Frame>
...
<Steps>
  ...
  <Step title="Configure double-booking setting">
    1. In the **Connected Coaches** section, locate the checkbox "Check this box to allow bookings for this session even if the connected coaches are already scheduled for other sessions at the same time"
...
**Location setting** specifies where sessions from this scheduler occur: a physical address, or a **Google Meet** or **Zoom** meeting link for sessions run over video. Athletes see the address or the join link on the booking, so everyone knows where to show up.

The location you pick in the scheduler's **Location** section (**Select Location** or **Add Location**) is the **default** for every date. If you train at more than one site with the same session, you do not need a scheduler per site: give the individual dates their own venue in the scheduler's availability, and each falls back to this default until you change it. How to do that: [A different venue for each date](/scheduling/setup-locations#a-different-venue-for-each-date).
...
- ✅ A **Google Meet** or **Zoom** meeting link (see [Virtual locations](/scheduling/setup-locations#virtual-locations-google-meet-and-zoom))
...
<Tabs>
  <Tab title="Studio/Facility-Based Training">
    ...
    **Best practice**: Include parking instructions, entrance details in scheduler description (not location field)
    ...
    <Tip>
      **Add helpful details**: In your scheduler description, mention parking, which entrance to use, or check-in procedures. Location field is for the address or meeting link only.
    </Tip>
  </Tab>
  ...
  <Tab title="Virtual/Online Training">
    **Your setup**: Sessions via video call (Zoom or Google Meet)

    **Configuration**:

    - Save a location whose address is your Google Meet or Zoom link, and attach it to the scheduler
    - CoachIQ labels the provider and shows joining instructions and a join action on the booking in the athlete portal, on your website, and in the native app
    - An invalid meeting link is blocked when you save, so athletes never get a dead link
    - Use one reusable meeting link per scheduler; if you generate a fresh link per call, update the location before the session

    How to set it up: [Virtual locations](/scheduling/setup-locations#virtual-locations-google-meet-and-zoom)
    ...
    **Best practice**:
    ...
    <Tip>
      Save Google Meet or Zoom links as virtual locations, then attach the location to the scheduler. Confirm the joining details in the athlete-facing booking view before sharing the session.
    </Tip>
  </Tab>
</Tabs>
...
<Frame>
  ![Location settings in scheduler builder](/images/location-scheduler-ui.png)
</Frame>
...
<Steps>
  ...
  <Step title="Add contextual details (optional)">
    While the location field holds only the address or meeting link, use your **scheduler description** to add helpful context:
...
<AccordionGroup>
  ...
  <Accordion title="Virtual link buried in the description">
    **Mistake**: Setting the location to "Online" and pasting `https://zoom.us/j/123456789` into the scheduler description or a reminder

    **Problem**:

    - Athletes hunt for the link at session time
    - A changed link means editing every message that carried the old one
    - Nothing checks that the link is valid

    **Fix**:

    - Save the Google Meet or Zoom link as a location and attach it to the scheduler
    - The provider label, joining instructions, and join action then appear on every booking automatically
    - See [Virtual locations](/scheduling/setup-locations#virtual-locations-google-meet-and-zoom)
  </Accordion>
</AccordionGroup>
...
**Important note**: Reminders are set **per scheduler**. Configure reminders on the scheduler you are editing, and check each scheduler you use. Editing one scheduler's reminders does not change the others.
...
<Steps>
  <Step title="Timing options">
    ...
    **Custom**: Set any number of days, hours, and minutes before the session (e.g., 2 hours before)

    **Multiple reminders**: You can add more than one reminder to a scheduler
  ...
  <Step title="Delivery methods">
    **Available methods**:
    ...
    - ✅ **SMS** (text message)
    - ✅ **Announcement** (sent like an Announcement, by email and push notification)
    - ✅ **In-App** (in-app message)

    **You can choose**:

    - A single method (SMS only)
    - Multiple methods (SMS + Announcement)
    - All three methods (SMS + Announcement + In-App)

    **Recommendation**: Use at least two methods (SMS + Announcement) for maximum reach
...
<Tabs>
  ...
  <Tab title="Channel Selection">
    ...
    **Announcement only** (email and push):
    ...
    **SMS + Announcement + In-App**:

    - ✅ Covers all bases
    - ✅ Athletes also see the reminder in their in-app messages
    - ❌ May feel like overkill to some athletes

    **Recommended**: Start with SMS + Announcement for critical reminders (24hr before), and add In-App if your athletes use the app
  </Tab>
</Tabs>
...
<Frame>
  ![Reminder configuration with timing and delivery method options](/images/Reminders-UI.png)
</Frame>

<Steps>
  <Step title="Access reminder settings">
    **Navigation**: Schedule → Schedulers → edit the scheduler → **Reminders** (toward the bottom of the scheduler settings)

    (Note: Reminders are per scheduler. Repeat these steps on each scheduler that needs them.)
  </Step>
  <Step title="Set timing">
    1. Add a reminder in the **Reminders** section
    2. Under **Time Before**, pick 30 mins, 1 hour, 1 day, 2 days, or **Custom** (set days, hours, and minutes)
    3. Add another reminder to send more than one
  ...
  <Step title="Choose delivery channels">
    Under **Method**, select one or more for each reminder:

    - In-App
    - Announcement
    - SMS

    **Recommendation**: Check SMS and Announcement for both reminders
  </Step>
  ...
  <Step title="Save and test">
    1. Save your changes to the scheduler
...
**What it is**: The **Session lengths** control at the top of **Advanced settings**

**Already covered**: Duration is configured during scheduler creation ([Creating Your First Scheduler](/scheduling/creating-your-first-scheduler))

**Options**:

- Preset durations: **15 / 30 / 45 / 60 mins**
- **Custom**: enter an exact duration in hours and minutes

Two related controls sit next to Duration: **New sessions can start** (how often a start time is offered) and **Allow overlapping sessions**. They are explained in [Duration, start times, and overlapping sessions](/scheduling/overlapping-slots).
...
<Note>
  **Note**: Changing duration ONLY affects future bookings. Existing bookings retain their original duration. To change the length of one already-booked occurrence, use **Duration → Edit** on the session's calendar click view; see [Edit duration from the calendar](/scheduling/editing-rescheduling-bookings#edit-duration-from-the-calendar).
</Note>

### Offer more than one session length

Use one scheduler for choices such as **30 or 60 mins**. In the scheduler builder, open **Advanced settings → Session lengths**, select the lengths you want to offer, then save the scheduler.

Athletes choose a length before choosing a date and time. The available times match that length; changing the length clears the previous date and time selection. Capacity, coach and resource conflicts, buffers, and booking limits still apply.

For example, a 4–5 PM window can offer 4–4:30 PM and 4:30–5 PM for a 30-minute choice, or 4–5 PM for a 60-minute choice. A longer option needs enough uninterrupted availability to fit.

<Frame>

  ![Session lengths with 30 and 60 minutes selected in the scheduler builder](/images/scheduler-session-lengths.png)

</Frame>

Set a cash-price product and/or credit cost for each length in **Payment → Price by session length**. A blank override uses the normal scheduler price and credit cost. See [Price each session length](/scheduling/setting-up-payment-options#set-a-price-for-each-session-length).

This gives the athlete a choice of length. To set a fixed length for just one practice date, use the per-date setting below.

### Preview a specific date

In the scheduler builder's **Availability** section, the specific-dates table shows each connected date's hours, session length, and resources. Use **All**, **Upcoming**, or **Past** to narrow the list.

Select a date to move the booking preview to that month and day. If it has no open times, the preview stays on your selection and shows **No bookable times on this date**. Check the date's hours and any blocks before sharing the scheduler.

To add or change dates, select **Open availability** on the row. It opens the availability template on that exact date in a new tab, keeping your unsaved scheduler work in place. Date-level **Session length** and **Resources** controls each have their own **Save** button.

### A different duration on one date

A single scheduler can run a 90-minute indoor practice on Tuesday and a 150-minute field practice on Saturday. Instead of a scheduler per length, give the date its own duration:

<Steps>
  <Step title="Open Availability → Specific-date settings">
    In the scheduler builder, go to **Availability**. Below the availability pickers is **Specific-date settings** ("Adjust resources or duration for one configured date").
  </Step>
  <Step title="Pick the date">
    The list shows the specific dates on the availability templates connected to this scheduler. If it reads "No specific dates are connected", add specific-date availability to a connected template first (see [Create your availability](/scheduling/create-your-availability#method-2-specific-dates-availability)), then refresh the scheduler.
  </Step>
  <Step title="Set the duration for that date">
    Enter the minutes under **Session length** and click its **Save** button. Use **Restore default** to remove the override. Every other date keeps the scheduler's default. You can change the date's resources on the same screen; see [Resources](/scheduling/resources#different-resources-on-different-dates).
  </Step>
  <Step title="Check the preview">
    Confirm the saved duration in the selected date's preview. Slots and coach/resource conflict checks for that date use its own length. Save any other scheduler changes with **Update** before leaving.
  </Step>
</Steps>

<Warning>
  **"Window shorter than duration" save error.** A date's time window must be at least as long as the duration that applies to it. A 5:30–7:00 PM date (90 minutes) on a 120-minute scheduler is rejected when you save, with an error naming the problem, instead of silently producing a date with no bookable slots. Fix it by shortening the duration for that date or widening the window on the availability template.
</Warning>
...
<AccordionGroup>
  <Accordion title="✅ Coach Assignment Verified">
    **Check**:
    ...
    - The Connected Coaches same-time booking checkbox is UNCHECKED (unless specific need)
  ...
  <Accordion title="✅ Reminders Configured">
    **Check**:
    ...
    - Reminders added on each scheduler that needs them
    - At least 2 reminders configured (24hr + 1hr recommended)
    - SMS and/or Announcement methods selected
...
### 2. Wrong venue on some dates

**Symptom**: Athletes show up at the default location on a date that runs somewhere else

**Fix**: Set the venue on each date that differs from the scheduler's default (see [Locations](/scheduling/setup-locations#a-different-venue-for-each-date)), and check the athlete's schedule shows the per-date venue. Only split into separate schedulers if you want to compare the sites in Calendar analytics

### 3. Same-time double-booking accidentally allowed

**Symptom**: Same coach booked in two places at once, scheduling conflicts

**Fix**: Edit scheduler → **Connected Coaches** section → uncheck "Check this box to allow bookings for this session even if the connected coaches are already scheduled for other sessions at the same time" → Update

### 4. Reminders not configured

**Symptom**: High no-show rates, athletes forgetting sessions

**Fix**: Schedule → Schedulers → edit each scheduler → **Reminders** → add 1 day and 1 hour reminders
...
- Missing reminders → Add reminders on that scheduler
...
<CardGroup cols={2}>
  <Card title="Scheduling Overview" icon="folder-open" href="/scheduling/overview">
  ...
  <Card title="Multi-Coach Sessions" icon="users" href="/scheduling/managing-availability-for-multiple-coaches">
...
<CardGroup cols={3}>
  ...
  <Card title="Scheduler Limits & Permissions" icon="cog" href="/scheduling/scheduler-settings-limits-and-permissions">
  ...
  <Card title="Blackout Days" icon="ban" href="/scheduling/setting-black-out-dates">
    Block out unavailable dates
  </Card>
  <Card title="Credit System" icon="tools" href="/payments/credits/credit-system-overview">
