...
<Steps>
  ...
  <Step title="Set the duration for that date">
    Enter the duration for that date only. Every other date keeps the scheduler's default. You can change the date's resources on the same screen; see [Resources](/scheduling/resources#different-resources-on-different-dates).
  </Step>
  <Step title="Update">
    Click **Update**. Slots for that date are generated with its own length, and coach and resource conflict checks for that date use it too.
  </Step>
</Steps>
