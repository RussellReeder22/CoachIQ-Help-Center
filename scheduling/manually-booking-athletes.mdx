...
icon: "user-plus"
keywords: ["manual booking", "book athlete", "add booking", "booking builder", "book for athlete", "coach book session", "book by tag", "tags"]
---
...
<Info>
  **Where to find this**: **Schedule** → **Calendar** → **+ Book Session**. That opens **New Booking**.
</Info>
...
<Tabs>
  <Tab title="From Calendar">
    **Best for**: Booking at a specific time you can see on calendar
    ...
    2. Click **+ Book Session** (top right)
    3. **New Booking** opens
...
New Booking has these sections: **Client**, **Scheduler**, **Date & Time**, **Recurring**, and **Notes**.

<Frame>
  ![CoachIQ Booking Builder interface with client, scheduler, and date selection](/images/manual-booking.png)
</Frame>

<Steps>
  <Step title="Select the client">
    On **Client**, the field placeholder is **Search users**. The dropdown is grouped into **Tags** and **People**.

    You can pick a person under **People**, or pick a tag under **Tags**. Clicking a tag adds a removable chip above the field. People in your group who have that tag show a checkmark. There is no expand modal and no athlete-count copy.

    <Frame>
      ![New Booking Client dropdown grouped into Tags and People](/images/scheduling-new-booking-tags.png)
    </Frame>

    <Frame>
      ![Selected tag chip above Search users, with a checkmark on a matching person](/images/scheduling-new-booking-tag-selected.png)
    </Frame>
  </Step>
  ...
  <Step title="Review and create booking">
    ...
    **If everything looks correct**, click **"Create"** or **"Book Session"**.
    ...
    <Note>
      **Recurring conflict warning.** When the booking is recurring and one of its occurrences overlaps another booking on the same session, CoachIQ tells you which booking it collides with and asks you to confirm or cancel. Confirm only if the double-up is intentional; on a capacity-1 session it puts two athletes in the same slot. Details in [Recurring bookings](/scheduling/recurring-bookings#creating-a-recurring-booking-pattern).
    </Note>
...
<Steps>
  <Step title="Access Copilot">
    ...
    <Frame>
      ![CoachIQ Copilot panel for natural language booking input](/images/copilot-ui.png)
    </Frame>
  </Step>
...
- [**Managing Athlete Credits**](/payments/credits/managing-athlete-credits) - Understanding credit balances, transactions, and manual adjustments
