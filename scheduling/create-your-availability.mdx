...
icon: "calendar-plus"
keywords: ["availability", "create availability", "set hours", "working hours", "when can athletes book", "schedule availability"]
---
...
<Steps>
  <Step title="Navigate to Availability">
    From your dashboard:

    1. Click **Scheduling** in the main navigation
    2. Click **Schedule** from the submenu
    3. Select the **Availability** tab in the left sidebar
  </Step>
  <Step title="Create new Availability">
    Click **+ New availability** to open the availability builder.

    <Frame>
      ![Schedule Availability tab showing availability templates](/images/scheduling/schedule-availability-list.png)
    </Frame>
  </Step>
  <Step title="Select your available days" stepNumber={3}>
    ...
    **Example 3: Flexible Schedule**
    ...
    <Frame>
      ![CoachIQ availability schedule showing selected days with time ranges](/images/scheduling/availablity-example.png)
    </Frame>
  </Step>
...
Use specific dates for camps, seasonal coaching, or irregular practices. Desktop shows a compact table; on a phone, each date has a card showing its hours, availability, location, and resources.

<Steps>

  <Step title="Open Specific Dates">

    Open the availability template and choose **Specific Dates**. Use the filter to find dates already on the schedule.

  </Step>

  <Step title="Add or edit a date">

    Select **Add date** beside the section heading to create an entry, or select an existing date to open its editor. Enter the date and start and end times, then choose **Done**.

  </Step>

  <Step title="Update several dates together">

    Use **Select dates** to select the entries you want to change together. Check the selected dates before applying a bulk edit. Individual dates can have different hours and venues.

  </Step>

  <Step title="Review and save">

    Check the dates, hours, and locations, then choose **Update** to save the availability. Blank or incomplete dates show a warning and cannot be saved; complete or remove those entries first.
...
<Note>
  ...
  Specific dates do not extend automatically. Add new dates as your schedule grows. A connected scheduler can also override duration and resources for one date under **Availability → Specific-date settings**. See [Per-date venues](/scheduling/setup-locations#a-different-venue-for-each-date) and [Per-date duration](/scheduling/advanced-scheduler-settings#a-different-duration-on-one-date).

</Note>

<Frame>

  ![Specific-date editor showing start and end times, location, resources, and Done; Update saves the availability](/images/specific-date-editor-2026-09.png)
...
<Steps>
  <Step title="Access Copilot in scheduler builder">
    ...
    <Frame>
      ![Availability builder with Copilot panel on the right side](/images/scheduling/availablity-example.png)
    </Frame>
  </Step>
...
<Steps>
  <Step title="Navigate to your scheduler">
    1. Go to **Scheduling → Schedule → Schedulers tab**
...
<CardGroup cols={2}>
  <Card title="Create Your First Scheduler" icon="sliders-h" href="/scheduling/creating-your-first-scheduler">
    Use your availability to create bookable session types
  </Card>
  <Card title="Managing Blackout Days" icon="calendar-times" href="/scheduling/setting-black-out-dates">
    Advanced guide to blocking time off and managing exceptions
  </Card>
  <Card title="Multi-Coach Availability" icon="users" href="/scheduling/managing-availability-for-multiple-coaches">
    Set up availability for teams and multiple coaches
  </Card>
  <Card title="Schedulers Dashboard" icon="th-large" href="/scheduling/what-are-schedulers">
...
## Start dates and minimum notice

The scheduler's start date is a calendar-date boundary, not a minimum-notice cutoff. Early times on the first date can be available when they satisfy the saved hours, duration, conflicts, and notice rules. If a configured date is missing, compare the date range and Specific-date settings with the booking preview.

## Shared availability and different session lengths

If several schedulers share one availability, a specific-date window can save when at least one linked session length fits. For example, a 90-minute window shared by a 60-minute lesson and a 180-minute camp offers lesson times but no camp times. A window that fits none of the linked sessions is rejected. Past dates do not block saving new availability; per-date custom lengths still need to fit their windows.
