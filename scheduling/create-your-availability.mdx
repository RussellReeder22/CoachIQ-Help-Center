...
<Steps>
  <Step title="Navigate to scheduler creation">
    From your dashboard:

    1. Click **Schedule** in the main navigation
    2. Click **Schedulers** from the submenu
    3. Click **Create** → **Create Scheduler**

    <Note>
      **Important**: Availability is configured _within_ the scheduler builder, not as a separate standalone feature. When you create or edit a scheduler, you'll set its availability there.
    </Note>
    <Frame>
      ![Navigation to scheduler builder](/images/navigate-to-scheduler-builder.png)
    </Frame>
  </Step>
  <Step title="Scroll to the Availability section">
    In the Scheduler Builder, scroll down to find the **Availability** or **Schedule** section.

    You'll see options for:

    - Adding availability patterns
    - Selecting days of the week
    - Setting time ranges

    <Frame>
      ![Availability section in scheduler builder](/images/availability-section-scheduler-builder.png)
    </Frame>
  </Step>
  <Step title="Choose 'Weekly Recurring' pattern">
    Look for an option to select between:

    - **Weekly Recurring** or **Recurring Schedule**
    - **Specific Dates** or **Custom Dates**

    Select **Weekly Recurring** (or similar option).

    <Tip>
      **Why weekly recurring?** It's set-and-forget. You define the pattern once (M/W/F 3-8 PM), and CoachIQ automatically generates availability for all future weeks.
    </Tip>
  </Step>
  <Step title="Select your available days">
    ...
    **Example 3: Flexible Schedule**
    ...
    <Frame>
      ![Day of week selection checkboxes](/images/select-available-days.png)
    </Frame>
    <Warning>
      **Double-check your selections**: It's easy to accidentally check or miss a day. Verify your selected days before moving to the next step.
    </Warning>
  </Step>
  <Step title="Set time ranges for each day">
    ...
    **Or same hours across all days**:
    ...
    <Frame>
      ![Time range selectors for each day](/images/set-time-ranges-per-day.png)
    </Frame>
    <Tip>
  ...
  <Step title="Account for buffer time in your end time">
    **Important math**: If you want your last session to end by 8:00 PM and you're using 15-minute buffer time, set your availability to end at 8:15 PM or 8:30 PM.

    **Example calculation**:

    - Last session starts: 7:00 PM
    - Session duration: 60 minutes
    - Session ends: 8:00 PM
    - Buffer time: 15 minutes
    - **Set availability end time**: 8:15 PM

    This ensures your 7:00 PM session has proper buffer time.

    <Note>
      **Why this matters**: If your availability ends exactly when you want sessions to end, the buffer time will cut into your availability window, potentially preventing the last session from being bookable.
    </Note>
  </Step>
  <Step title="Name your availability template (if option available)">
    Some systems let you name your availability template for easy reference later.
    ...
    **Avoid generic names**:
    ...
    <Frame>
      ![Template naming field](/images/name-availability-template.png)
    </Frame>
  </Step>
...
<Steps>
  <Step title="Access the Availability section">
    In the Scheduler Builder, navigate to the **Availability** section (same as Method 1, Steps 1-2).
  </Step>
  ...
  <Step title="Select your available dates">
    ...
    **Irregular schedule**:
    ...
    <Frame>
      ![Calendar date picker for specific dates](/images/specific-date-picker.png)
    </Frame>
    <Tip>
  ...
  <Step title="Set time ranges for selected dates">
    ...
    **Option B: Different times per date** (if supported)
    ...
    <Frame>
      ![Time ranges for specific dates](/images/specific-date-time-ranges.png)
    </Frame>
  </Step>
  <Step title="Review and save">
    Verify:
...
<Steps>
  <Step title="Access Copilot in scheduler builder">
    Look for a **Copilot** button, icon, or panel in the Scheduler Builder.

    Copilot is typically available in the Availability section or as a side panel.
  ...
  <Step title="Describe your availability in plain language">
    ...
    **Location-based**:
    ...
    > "Set up availability for my downtown gym: Tuesday and Thursday evenings 5-9 PM"
  ...
  <Step title="Accept and save">
    Once you're satisfied with the generated availability:

    1. Click **Accept** or **Use This Availability**
    2. Continue with the rest of your scheduler configuration
    3. Save your scheduler

    Your Copilot-generated availability is now active and ready to generate bookable slots.
  </Step>
</Steps>
