...
Offer 30-minute and 60-minute sessions with separate prices or credit costs. Follow [Price by session length](/scheduling/pricing-by-session-length) for the current setup screens, default behavior, and booking checks.
...
<CardGroup cols={2}>
  ...
  <Card title="Test Every Change" icon="vial">
    After any payment configuration change, complete a test booking yourself. Only use Stripe test cards in a confirmed Stripe test environment. For a live account, review the checkout summary without submitting a payment.
  </Card>
