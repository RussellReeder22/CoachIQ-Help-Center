...
## Set a price for each session length

When your scheduler offers multiple lengths, its **Payment** section includes **Price by session length**.

1. Choose the lengths in **Advanced settings → Session lengths**.
2. In **Payment → Price by session length**, choose a cash-price product and/or credit cost for each length, matching the payment methods your scheduler accepts.
3. Save the scheduler and check each length in the booking preview.

For example, 30 minutes can cost $50 or 1 credit, while 60 minutes costs $100 or 2 credits. These are separate settings; CoachIQ does not assume an hourly rate. A blank length override uses the scheduler's normal product and credit cost.

Changing the selected length clears the previous time, refreshes availability, and updates the price and credit requirement. Existing bookings retain what was charged. A reschedule cannot switch to a differently priced length without a new purchase: cancel under your cancellation policy and book the new length instead.
