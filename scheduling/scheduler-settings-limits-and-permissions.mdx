...
### Example 1: Starter Coach (Jake) - Private Training
...
### Example 2: Scaling Coach (Chris) - Tiered Group Classes

**Business context**:

- 3 group class offerings at different intensity levels
- 40 total class slots per week
- 60 active athletes across all levels

**Scheduler configuration**:

**"Rising Stars Class" (Beginner)**

```
Capacity: 12 athletes
Booking limits:
  - Max per day: 2 classes (morning + evening)
  - Max per week: 3 classes
Cancellation: 24-hour notice, refund credits
Buffer: 20 minutes after (larger cleanup for group)
Windows: 24-hour min, 30-day max
Multiple bookings: Yes
```

**"Elite Performance Class" (Advanced)**

```
Capacity: 8 athletes (smaller for quality)
Booking limits:
  - Max per day: 1 class (high-intensity)
  - Max per week: 4 classes
Cancellation: 24-hour notice, refund credits
Buffer: 20 minutes after
Windows: 24-hour min, 30-day max
Multiple bookings: Yes
```

**"Open Gym Access" (Drop-in)**

```
Capacity: 20 athletes
Booking limits: No limits (unlimited)
Cancellation: Anytime cancellation allowed
Buffer: 5 minutes after
Windows: Same-day booking allowed (6-hour min), 14-day max
Multiple bookings: Yes
```

**Why this works**:

- Different capacities match service intensity
- Booking limits ensure fair access to most popular classes
- Open gym has no limits to encourage frequent attendance
- Tiered system allows Chris to serve 60 athletes across 40 slots
- Buffer times account for group class cleanup requirements

---

### Example 3: Facility Owner (Mike) - Multi-Service Facility

**Business context**:

- 3 coaches on staff
- Private training + group classes + assessments
- 100+ athletes across all services

**Configuration approach**: Different schedulers for different services

**"Private Training Sessions"**

```
Capacity: 1 athlete
Limits: 3 sessions/week, 1/day
Cancellation: 48-hour notice (premium service), refund credits
Buffer: 10 min before, 15 min after
Windows: 48-hour min, 30-day max
Multiple bookings: Yes
```
...
```
Capacity: 25 athletes
Limits: No weekly limit (encourage frequent attendance)
Cancellation: 6-hour notice, refund credits
Buffer: 30 min after (full room reset)
Windows: Same-day allowed (6-hour min), 14-day max
Multiple bookings: Yes
```
...
```
Capacity: 1 athlete
Limits: 1 assessment per athlete per month
Cancellation: 72-hour notice, no refund (extensive prep required)
Buffer: 30 min before, 30 min after (detailed prep/debrief)
Windows: 7-day min, 60-day max
Multiple bookings: No (must complete assessment first)
```

**Why this works**:

- Each service has appropriate rules for its context
- Premium services have stricter policies
- High-volume classes encourage frequent attendance
- Assessment scheduler reflects specialized nature and prep requirements
- Scaled appropriately for 100+ athlete facility
...
## Optimizing settings over time

Your initial settings won't be perfect. Use this optimization process:
...
- Cancellation rate?
- No-show rate?
- Athlete feedback on booking experience?
- Your personal exhaustion level?

**Don't change yet**: Collect data first
...
<Tabs>
  <Tab title="High Cancellation Rate (>25%)">
    **Possible fix**:

    - Tighten cancellation policy (24-hour → 48-hour)
    - Remove credit refunds for late cancellations
    - Add booking limits to reduce frivolous bookings

    **Monitor**: Does cancellation rate improve?
  </Tab>
  <Tab title="Low Booking Volume">
    **Possible fix**:

    - Reduce minimum notice (24-hour → 12-hour)
    - Increase future booking window (30 days → 60 days)
    - Allow more bookings per week
    - Reduce cancellation strictness

    **Monitor**: Does booking volume increase?
  </Tab>
  <Tab title="Burnout/Exhaustion">
    **Possible fix**:

    - Increase buffer time (10 min → 15 min)
    - Add buffer before sessions (if only had after-buffer)
    - Reduce max bookings you allow per week
    - Add specific time blocks with no bookings

    **Monitor**: Do you feel more sustainable?
  </Tab>
  <Tab title="Unfair Access">
    **Possible fix**:

    - Add weekly booking limits (currently unlimited → 3/week)
    - Add daily limits if one athlete booking multiple same-day
    - Consider tiered schedulers for different athlete levels

    **Monitor**: Are more athletes able to book?
  </Tab>
</Tabs>
...
1. Are settings achieving desired athlete behavior?
2. Is revenue stable/growing?
3. Are you sustainable/not burned out?
4. Are athletes happy with booking experience?
5. Do settings match your current business stage?

**Adjust as you scale**:

- Starter coach → Scaling coach: Tighten policies (more demand)
- Scaling coach → Facility owner: Service-specific settings
- Low demand → High demand: Add limits and stricter windows

---

## Quick reference: Recommended settings by coaching stage

### Starter Coach (Jake) - Building Clientele

```yaml
Capacity: 1 athlete (private training)
Booking Limits:
  Per Day: 1 session
  Per Week: 2 sessions
Cancellation: 24-hour notice, refund credits
...
**Philosophy**: Balance athlete flexibility with basic business protection

---

### Scaling Coach (Chris) - Optimizing Operations

```yaml
Private Training:
  Capacity: 1 athlete
  Limits: 1/day, 3/week
  Cancellation: 24-48 hour, refund within window
...
Group Classes:
  ...
  Limits: 2/day, 4/week
  Cancellation: 24-hour, refund credits
  Buffer: 20 min after (cleanup)
  Windows: 12-hour min, 14-day max
```

**Philosophy**: Service-specific settings optimized for different offerings

---

### Facility Owner (Mike) - Scale & Systematize

```yaml
Premium Private:
  Capacity: 1 athlete
  Limits: 3/week, 1/day
  Cancellation: 48-hour, refund within window
  Buffer: 15 min before, 15 min after
  Windows: 48-hour min, 30-day max

Standard Group:
  Capacity: 20 athletes
  Limits: No weekly limit
  Cancellation: 24-hour, refund
  Buffer: 30 min after
  Windows: 6-hour min, 14-day max

Assessments:
  Capacity: 1 athlete
  Limits: 1 per month
  Cancellation: 72-hour, no refund
  Buffer: 30 min both sides
  Windows: 7-day min, 60-day max
```

**Philosophy**: Tiered policies reflecting service premium levels and prep requirements
