...
icon: "lock"
keywords: ["booking limits", "capacity", "cancellation policy", "permissions", "max bookings", "booking window", "advance notice"]
---
...
Cap how often a scheduler can be booked, in total and per athlete.

<Info>
  **Where to find this**: Scheduler → **Advanced settings** → the **Session-wide booking limits**, **Time slot capacity**, and **Per-athlete booking limits** groups
</Info>

### What it does

**Booking limits** come in two scopes, and the Advanced settings section labels them so you can tell which is which:

- **Session-wide booking limits** cap total bookings for this session across **all** athletes combined.
- **Per-athlete booking limits** cap how many bookings **each** athlete can make for this session.

Plus **Time slot capacity**, which is how many athletes fit in one time slot.

### Session-wide booking limits

*Total bookings for this session across all athletes*

<Steps>
  <Step title="Max total bookings per day">
    Maximum times this scheduler can be booked in a single day, counting every athlete and every time slot.

    Use it when you only want to run a limited number of a session type each day, for example accepting one private session a day regardless of who books it. It is not a per-athlete cap; two different athletes count as two bookings.
  </Step>
  <Step title="Max total bookings per week">
    Maximum times this scheduler can be booked within a 7-day rolling period, counting every athlete and every time slot.
  </Step>
</Steps>
...
<Steps>
  <Step title="Max athletes per time slot (most common)">
    Maximum athletes that can book one available time slot. This is the setting that makes a session private (1), small group (4–6), or large group. It is the same control described in [Setting 1: Session capacity](#setting-1-session-capacity).
  </Step>
</Steps>

### Per-athlete booking limits

*Maximum bookings each athlete can make for this session*

<Steps>
  <Step title="Max bookings per athlete per day">
    How many times one athlete can book this scheduler on a single day. Leave blank for no per-athlete cap.
  </Step>
  <Step title="Max bookings per athlete per week">
    How many times one athlete can book this scheduler in a 7-day rolling period.
  </Step>
</Steps>

Per-athlete limits exist for unlimited memberships, open gym, and rentals, where one athlete could otherwise grab every open slot. A typical setup for an unlimited membership: **Max bookings per athlete per day 1**, **Max bookings per athlete per week 5**, with no session-wide cap so the facility itself stays fully bookable.

**How the two scopes combine**: a booking has to pass both. With **Max total bookings per day 10** and **Max bookings per athlete per day 2**, the tenth booking of the day closes the scheduler for everyone, and an athlete who already has two bookings today is turned away even when only three of the ten are taken.

<Note>
  Per-athlete limits are different from **Allow athletes to book a timeslot more than once** (Setting 7). That toggle decides whether the same account can hold two spots in the *same* time slot (a parent booking two children). Per-athlete limits count an athlete's bookings *across* time slots over a day or week.
</Note>

<Tip>
  Coaches and admins booking from the calendar are not stopped by these limits, so you can always add a make-up session by hand. See [Manually booking athletes](/scheduling/manually-booking-athletes).
</Tip>
...
<Frame>
  ![Scheduler Booking Rules showing all six permission toggles](/images/scheduler-booking-rules.png)
</Frame>
...
<Note>
  **Booking without an account.** A scheduler can also be opened to guest checkout, so a prospect can reserve an intro call or trial session with just their contact and payment details and no login. Where the toggle lives, what a guest provides, and how the booking shows up: [Guest checkout](/payments/guest-checkout).
</Note>

## Setting 8: Duration, start times, and overlapping sessions

Control how long a session runs, how often a new one can start, and whether two bookings may share a window.

<Info>
  **Where to find this**: Schedule → Schedulers → [Select Scheduler] → **Advanced settings**
</Info>

These are three independent controls at the top of Advanced settings:

| Control | What it does |
|---|---|
| **Duration** | How long one booked session runs (**15 / 30 / 45 / 60 mins** or **Custom**) |
| **New sessions can start** | How often a start time is offered. Usually the same as duration; make it shorter to offer more start times (a 60-minute lesson with 30-minute starts offers 9:00–10:00 or 9:30–10:30) |
| **Allow overlapping sessions** | Checkbox. Off: a booking removes every later start time that would collide with it, so once 9:00 is booked, 9:30 drops out. On: two bookings of this session may occupy the same window |

Setting a short start cadence does not turn overlap on. Leave **Allow overlapping sessions** unchecked and athletes simply get more start times to choose from, with collisions removed as bookings come in.

**Example**: A 60-minute "Youth Skills" session with **New sessions can start** at 15 minutes and **Allow overlapping sessions** checked offers start times at 10:00, 10:15, 10:30, and so on — each a full 60-minute session that overlaps the next, each with its own capacity.

Full walkthrough with example setups: [Duration, start times, and overlapping sessions](/scheduling/overlapping-slots).

<Note>
  Overlapping sessions is different from **session capacity** (Setting 1). Capacity controls how many athletes share the *same* start time; overlap controls whether a *new* start time can be booked while another session from this scheduler is running.
</Note>
