...
<Steps>
  ...
  <Step title="Enable the Recurring toggle">
    ...
    **What happens when enabled**: Additional configuration options appear:
    ...
    <Frame>
      ![Recurring toggle enabled in Booking Builder](/images/recurring-booking-ui.png)
    </Frame>
  </Step>
  ...
  <Step title="Use CoachIQ Copilot for quick setup (optional)">
    ...
    **Examples of what to type**:
    ...
    <Frame>
      ![CoachIQ Copilot panel for generating recurring booking schedules](/images/copilot-ui.png)
    </Frame>
  ...
  <Step title="Click Create and confirm">
    ...
    <Note>
      **Conflict warning.** If any occurrence in the series overlaps another booking on the same session, CoachIQ stops and tells you which booking it collides with before creating anything. You can confirm and proceed (a deliberate double-up for a special arrangement) or back out and adjust the pattern. On a capacity-1 session, proceeding puts two athletes in the same slot, so treat the warning as a real one unless you meant it.
    </Note>
...
<Frame>
  ![Occurrences calendar with Change and Skip options](/images/occurrences-ui.png)
</Frame>
...
- **Past dates can be changed too.** If a session actually ran at a different time than the record shows, open the past occurrence and **Change** it so attendance and billing history are accurate. Upcoming sessions are not affected.
...
<Note>
  **Skip works on past dates as well as future ones.** If a session in the past did not happen (snow day, athlete no-show you agreed to waive), skip that occurrence to correct the record without touching the upcoming sessions.
</Note>
...
<Steps>
  ...
  <Step title="Delete the pattern">
    ...
    **What happens**: Deletes the recurring booking pattern—all remaining future sessions are removed.

    <Warning>
      **This action is permanent**: All remaining future sessions are deleted. Cannot be undone.
    </Warning>

    <Note>
      **Past occurrences are kept.** Deleting or editing a series does not remove the sessions that already happened. They stay on the calendar and on the athlete's profile, so attendance and billing records are intact.
    </Note>
  </Step>
...
- Past sessions from the series remain in the athlete's history
...
<Note>
  **For detailed credit system information**, see [Credit System Explained](/payments/credits/credit-system-overview) and [Credit Usage Limits](/payments/credits/credit-usage-limits-restrictions).
</Note>
