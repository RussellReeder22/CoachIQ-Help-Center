...
icon: "user-group"
keywords: ["multiple coaches", "team scheduling", "coach availability", "staff availability", "team calendar", "assign coaches"]
---
...
<Tabs>
  <Tab title="Approach 1: Coach-Specific Schedulers">
    ...
    **Cons**:
    ...
    <Frame>
      ![Multiple schedulers each assigned to specific coach](/images/multi-coach-schedulers.png)
    </Frame>
  </Tab>
  <Tab title="Approach 2: Shared Schedulers with Multi-Coach Assignment">
    ...
    **Cons**:
    ...
    <Frame>
      ![Single scheduler with multiple coach assignments](/images/multi-connected-coaches.png)
    </Frame>
  </Tab>
...
<Steps>
  ...
  <Step title="Verify calendar separation">
    ...
    <Frame>
      ![Calendar filtered to show specific coach's bookings](/images/coach-schedule-cal.png)
    </Frame>
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Assign multiple coaches to the scheduler">
    ...
    <Frame>
      ![CoachIQ scheduler with multiple connected coaches assigned](/images/multi-connected-coaches.png)
    </Frame>
  </Step>
  ...
  <Step title="Decide what an &quot;All&quot; session blocks">
    When **Connected Coaches** is left on **All**, a checkbox below the picker controls whether the session needs every coach:

    - **Check this box if this session needs every coach**: a booking on this session blocks every coach at that time, including sessions assigned to one specific coach.
    - **Uncheck this box if any one of your coaches can run this session**: the session is only blocked by other "All" sessions, so it can run alongside individual lessons.

    The label shows **(your account default)** until you change it. Accounts that only ever assign one coach default to needing every coach; multi-coach accounts default to any one coach. Two "All" sessions can never be booked on top of each other.
  </Step>
...
<Steps>
  <Step title="Filter by individual coach">
    ...
    **How**:
    ...
    <Frame>
      ![CoachIQ calendar filtered to show a single coach's schedule](/images/coach-schedule-cal.png)
    </Frame>
  ...
  <Step title="View all coaches simultaneously">
    ...
    **How**:
    ...
    <Frame>
      ![Calendar showing all coaches' bookings](/images/calendar-week-view.png)
    </Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Coach's availability not showing correctly">
    ...
    **Causes**:
    ...
    - **Connected coach already booked elsewhere** (very common!) - If this coach is assigned to multiple schedulers and has sessions booked through one scheduler, those time slots won't appear for their other schedulers. This is CoachIQ's double-booking prevention working correctly. Athletes only see a coach-conflict message when this is the actual cause; a time that is outside the scheduler's availability shows a plain "no availability" message instead, so the message the athlete reports tells you which to check first
    ...
    **Debug method**: Filter calendar to only that coach and verify availability windows show properly. Look for sessions from other schedulers that might be blocking the times you expect.

    <Tip>
      **Example**: Coach Mike is connected to both "Private Training" and "Group Classes" schedulers. If he has a Group Class booked Monday 4-5pm, that slot won't appear for Private Training either—both schedulers share the same coach, so both are affected by his existing bookings.
    </Tip>
