...
<Steps>
  ...
  <Step title="Put them on schedulers">
    Open a scheduler, go to its **Coaches** section, and pick the coaches who can run it under **Select Coaches**. Choose specific coaches, or **All** for open sessions any staff member can take.
  </Step>
</Steps>

To put a coach on a single booked session rather than the whole scheduler, click the session on **Schedule → Calendar** and add the coach from the click view. That assigns them to that occurrence only. **Connect coaches** on this tab can also attach one or more coaches to one or more sessions in one step, so the coach on a practice can differ session to session.
