...
<Steps>
  <Step title="Create credit-issuing products (Payment side)">
    **Where**: Payments → Products → Product Builder
...
## Part 1: Creating credit-issuing products

Credit issuance happens in the **Product Builder** under Payments.

### Creating a One-Time Credit Package

<Steps>
  <Step title="Navigate to Product Builder">
    Go to **Payments → Products** in your admin dashboard.

    Click **Create Product** (or **\+ Add Product**).

    The Product Builder opens.
  </Step>
  <Step title="Choose product type: One-Time Payment">
    **Product type options:**

    - ✅ **One-Time Payment** ← Select this for credit packages
    - Subscription (for recurring credit issuance)
    - Access Code (for restricted purchases)

    **Why One-Time**: Issues all credits immediately upon purchase, athlete uses credits until depleted, then purchases another package.

    **Example use**: "10-Credit Package", "5-Credit Trial Pack", "20-Credit Premium Bundle"
  </Step>
  <Step title="Configure product details">
    Fill out essential product information:

    **Product Name**: _Required_

    - ✅ Good: "10-Credit Standard Package"
    - ✅ Good: "20-Credit Premium Bundle"
    - ❌ Avoid: "Training Credits" (no quantity specified)

    **Description**: _Optional but recommended_

    ```
    Get 10 training sessions for $450 - save $50!
    
    • Use for any 60-min private training session
    • Credits never expire
    • Best value for regular athletes
    • Flexible scheduling
    
    Save $5 per session vs pay-per-session pricing.
    ```

    **Price**: `$450.00`

    **Features**: _Optional - add bullet points_

    - "10 training sessions included"
    - "Use anytime within 6 months"
    - "Flexible scheduling"

    <Tip>
      **Naming best practice**: Always include credit quantity in product name so athletes know exactly what they're purchasing.
    </Tip>
  </Step>
  <Step title="Enable credit issuance">
    Navigate to the **Credits** section in Product Builder.

    **Toggle ON**: "Add credits once purchased"

    **Number of credits**: `10` (how many credits this product grants)

    **Result**: When athlete purchases this product, they immediately receive 10 credits in their account.

    <Frame>
      ![Credits configuration in Product Builder](/images/product-builder-credits-section.png)
    </Frame>
  </Step>
  <Step title="Configure credit expiration (optional)">
    Still in the **Credits** section:

    **Credit expiration options:**

    <Tabs>
      <Tab title="Never Expire (Recommended)">
        **Setting**: Leave expiration fields empty OR select "Don't expire"

        **Behavior**: Credits remain in athlete's account indefinitely

        **Best for**: Most coaches—builds goodwill, simple to manage, no refund disputes

        **Communication**: "Your credits never expire—use them whenever you're ready!"
      </Tab>
      <Tab title="Days Since Purchase">
        **Setting**: "Expire after X days from purchase"

        **Example**: `180` days (6 months)

        **Behavior**: Credits expire 180 days after purchase date, regardless of usage

        **Best for**: Encouraging consistent training, aligning with program lengths

        **Communication**: "Credits expire 6 months after purchase—use them regularly!"
      </Tab>
      <Tab title="Specific Date">
        **Setting**: "Expire on [date]"

        **Example**: "Expire on 12/31/2025"

        **Behavior**: All credits from this product expire on specific date

        **Best for**: Seasonal programs, year-end promotions

        **Communication**: "All credits expire December 31st—book before year-end!"
      </Tab>
    </Tabs>
    <Warning>
      **Legal compliance**: Some jurisdictions have laws about stored value expiration. Check local regulations before setting expiration policies.
    </Warning>
  </Step>
  <Step title="Configure redeemable schedulers (CRITICAL)">
    This step determines **which schedulers accept these credits**.

    Navigate to **Usage Limits** section in Product Builder.

    **"Credits can be redeemed for these sessions"**: Select schedulers

    <Tabs>
      <Tab title="All Schedulers (Master Credits)">
        **Configuration**: Leave scheduler selection BLANK or select all

        **Result**: Creates **Master Credits**—can be used for ANY credit-enabled scheduler
        ...
        **Best for**:
        ...
        - Simple credit systems
        - General-purpose packages
        - Maximum flexibility for athletes

        **Example**: "10-Credit Universal Package - Use for any session"
      </Tab>
      <Tab title="Specific Schedulers Only">
        **Configuration**: Select ONLY specific schedulers

        **Example**: Select "Rising Stars Class", "Rising Stars Shooting Lab", "Rising Stars Open Gym"

        **Result**: Creates **Scheduler-Specific Credits**—ONLY usable for selected schedulers

        **Athlete sees**:

        - "Rising Stars 10-Credit Membership"
        ...
        **Best for**:
        ...
        - Revenue sharing with other coaches
        - Restricting premium credits to premium sessions

        **Example**: "Elite Program Membership - 10 credits for Elite Class only"

        <Tip>
          **Strategic use**: Sell both types—general packages (master credits) for flexibility and premium memberships (scheduler-specific) for higher tiers.
        </Tip>
      </Tab>
    </Tabs>
    **Usage Limits (optional)**: Set maximum credits per scheduler

    - Example: "3 of 10 credits can be used for Premium Class"
    - Limits reset based on billing frequency
    - Advanced feature for complex membership models

    <Check>
      **Critical**: If you don't select schedulers here, create Master Credits. If you select specific schedulers, those are the ONLY sessions this product's credits can book.
    </Check>
  </Step>
  <Step title="Configure advanced settings (optional)">
    ...
    **Processing Fees**:
    ...
    - Toggle: "Add processing fee"
    - Pass Stripe fees to customer (you receive full listed price)

    **Access Codes**:

    - Toggle: "Enable access codes"
    - Require special code to purchase (private memberships, partner programs)

    **Guest Checkout**:

    - Toggle: "Allow users to checkout as guest"
    - Purchase without creating account (limited scheduling features)

    **Purchase Limits**:

    - "Add a purchase limit for this product"
    - Restrict how many athletes can buy (capacity-limited programs)

    **Revenue Sharing**:

    - "Share revenue with another Stripe Account"
    - Split revenue with partner organization or facility

    **Terms Requirement**:

    - "Require Agreement to Terms"
    - Force athletes to accept terms before purchase (waivers, policies)

    **Confirmation Page**:

    - Direct users to athlete portal (default)
    - Direct users to thank you page
    - Direct users to custom URL

    <Note>
      **Start simple**: Only configure advanced settings if you have specific business needs. Basic setup works for 90% of coaches.
    </Note>
  </Step>
  <Step title="Save and publish product">
    Click **Save** or **Create Product** (usually top-right).

    Your credit-issuing product is now live!

    <Check>
      **Success**: Athletes can now purchase this product and receive 10 credits in their account. Next step: configure schedulers to accept these credits.
    </Check>
  </Step>
</Steps>
...
### Creating a Subscription with Recurring Credits

For monthly/weekly recurring revenue with automatic credit issuance:

<Steps>
  <Step title="Choose product type: Subscription">
    In Product Builder, select:

    - **Product Type**: Subscription ✅
    - (Not One-Time Payment)

    **Why Subscription**: Issues credits per billing cycle, auto-renews as long as subscription active, creates predictable recurring revenue.

    **Example use**: "Monthly Unlimited Membership", "Weekly Training Pass", "Quarterly Elite Program"
  </Step>
  <Step title="Set subscription frequency">
    ...
    **Example**: Select "Month" for monthly membership

    **Price**: `$250.00/month`

    **Result**: Athlete charged \$250 every month, receives credits every month
  </Step>
  <Step title="Configure recurring credit issuance">
    Navigate to **Credits** section:

    **Toggle ON**: "Add credits once purchased"

    **Number of credits**: `5` (credits issued PER BILLING CYCLE)

    **IMPORTANT**: This is credits **per month** (or week/year based on frequency), not total

    **Example**:

    - Frequency: Month
    - Credits: 5
    - Result: Athlete receives 5 fresh credits every month on renewal

    <Tabs>
      <Tab title="Finite Credits Per Period">
        **Configuration**: Set specific number (e.g., `5`)

        **Display to athlete**: "Credit Balance: 5"

        **Behavior**: Athlete receives 5 credits per month, uses them, gets 5 more next month

        **Best for**: Controlled usage, specific session limits, traditional membership models

        **Example**: "\$250/month - 5 Credits/month (up to 1-2x per week)"
      </Tab>
      <Tab title="Unlimited Credits Per Period">
        **Configuration**: Toggle "Unlimited credits" checkbox

        **Display to athlete**: "Credit Balance: ∞" (infinity symbol)

        **Behavior**: Athlete can book unlimited sessions during billing period

        **IMPORTANT**: `∞` means unlimited WITHIN current month—resets at next renewal, NOT unlimited forever

        **Best for**: High-frequency athletes, premium memberships, maximizing engagement

        **Example**: "\$400/month - Unlimited Training (book as many sessions as you want this month)"

        <Warning>
          **Common confusion**: ∞ is NOT unlimited forever. It's unlimited WITHIN the current billing period. Credits reset at renewal.
        </Warning>
      </Tab>
    </Tabs>
  </Step>
  <Step title="Configure unused credit behavior">
    **"Unused credits expire" setting**:

    <Tabs>
      <Tab title="Credits Expire (Don't Carry Over)">
        **Configuration**: Toggle "Unused credits expire" to ON

        **Behavior**: Unused credits disappear at end of billing cycle

        **Example**:

        - Month 1: Athlete receives 5 credits, uses 3 → 2 unused
        - Month 2: Athlete receives 5 fresh credits (2 from Month 1 expired)
        - New balance: 5 credits

        **Best for**: Encouraging consistent usage, preventing credit accumulation, simplified accounting

        **Communication**: "Use your 5 monthly credits or lose them—credits don't roll over!"
      </Tab>
      <Tab title="Credits Carry Over (Don't Expire)">
        **Configuration**: Toggle "Unused credits expire" to OFF

        **Behavior**: Unused credits accumulate indefinitely

        **Example**:

        - Month 1: Athlete receives 5 credits, uses 3 → 2 unused
        - Month 2: Athlete receives 5 fresh credits + 2 carried over
        - New balance: 7 credits

        **Best for**: Flexible usage, athlete-friendly, accounts for vacations/breaks

        **Communication**: "Unused credits roll over—take a break and they'll be waiting!"

        <Tip>
          **Most popular**: Carry over policy is athlete-friendly and reduces refund requests. Slight risk of credit accumulation but builds goodwill.
        </Tip>
      </Tab>
    </Tabs>
  </Step>
  <Step title="Configure redeemable schedulers">
    Same as One-Time products:

    Navigate to **Usage Limits** section.

    **Select schedulers** that accept these subscription credits:

    - Leave blank = Master Credits (any credit-enabled scheduler)
    - Select specific = Scheduler-Specific Credits (only those sessions)

    **Example for tiered membership**:

    - Basic Membership (\$150/month, 3 credits) → Can book "Group Classes" only
    - Premium Membership (\$300/month, 5 credits) → Can book "Group Classes" + "Private Sessions"
    - Elite Membership (\$500/month, unlimited ∞) → Can book ALL schedulers

    **Athlete sees**:

    - "Redeemable Sessions: Rising Stars Class (0/∞), Elite Class (0/∞)"
    - Per-scheduler usage tracking if usage limits set
  </Step>
  <Step title="Save subscription product">
    Click **Save** or **Create Product**.

    Subscription is now live!

    **Auto-renewal**: Athlete charged monthly via Stripe, credits auto-issued each billing cycle

    **What happens**:

    - Athlete subscribes → Receives credits immediately
    - Month 2 → Charged again, receives fresh credits (unused expire or carry over per your settings)
    - Athlete cancels → Subscription stops, remaining credits may become unusable (check your settings)

    <Check>
      **Success**: You now have a recurring revenue subscription that automatically issues credits each billing period.
    </Check>
  </Step>
</Steps>
...
## Part 2: Configuring schedulers to accept credits

Now that products issue credits, configure **which schedulers accept them** for booking.

<Steps>
  <Step title="Navigate to Schedulers">
    Go to **Schedule → Schedulers** in your admin dashboard.

    You'll see your list of existing schedulers.
  </Step>
  <Step title="Select scheduler to configure">
    Click on the scheduler you want to make credit-enabled.

    **Example**: "60-Min Private Training"

    The scheduler settings page opens.
  </Step>
  <Step title="Navigate to Payment/Pricing section">
    ...
    This section controls how athletes pay for this session type.
  </Step>
  <Step title="Enable credit payment">
    You'll see payment model options. Choose the model that fits your business:

    <Tabs>
      <Tab title="Credit-Only (Enforce Credits)">
        **Configuration:**

        - Set pricing model to **"Credits"** or **"Credit-Based"**
        - Enter credit amount: `1` (or 2, 0.5, etc.)
        - Do NOT enable direct payment option

        **Athlete requirement**: Must have credits to book—no other payment method accepted

        **System behavior**:

        - Athlete with 0 credits CANNOT book this scheduler
        - Booking page shows: "1 Credit Required"
        - If insufficient credits: "You need 1 credit to book. Purchase credits to continue."

        **When to use**:

        - Package-only business model
        - Want to enforce credit purchases
        - Simplify booking flow (no payment decisions)

        **Example**: "60-Min Private Training - 1 Credit (Required)"

        **Result**: Forces athletes into credit system, maximizes package sales
      </Tab>
      <Tab title="Hybrid (Credit OR Direct Payment)">
        **Configuration:**

        - Set pricing model to **"Hybrid"** or **"Credits + Payment"** or enable both
        - Enter credit amount: `1`
        - Enter direct payment amount: `$50.00`

        **Athlete choice**: Can use 1 credit OR pay \$50 direct—their choice at booking

        **System behavior**:

        - Athlete with credits sees: "Use 1 Credit OR Pay \$50"
        - Athlete with 0 credits sees: "Pay \$50" only
        - Booking page shows both options (if athlete has credits)

        **When to use**:

        - Offer flexibility to new athletes
        - Allow drop-ins without forcing package purchase
        - Maximize booking opportunities (don't block anyone)

        **Pricing strategy**: ✅ Set direct payment HIGHER than credit equivalent

        - 1 credit = \$45 value (in 10-pack)
        - Direct payment = \$50
        - \$5 premium incentivizes package purchase

        **Example**: "60-Min Private Training - 1 Credit OR \$50"

        **Result**: Flexibility + revenue optimization

        <Tip>
          **Recommended for most coaches**: Hybrid model maximizes bookings while still incentivizing packages through pricing psychology.
        </Tip>
      </Tab>
      <Tab title="Payment-Only (No Credits)">
        **Configuration:**

        - Set pricing model to **"Direct Payment"** or **"Pay Per Session"**
        - Enter price: `$75.00`
        - Do NOT enable credit payment option

        **Athlete requirement**: Must pay per session—credits cannot be used

        **When to use**:

        - Trial/assessment sessions (save credits for regular training)
        - Specialty services separate from packages
        - Premium one-off offerings
        - Events and workshops

        **Example**: "Initial Consultation - \$75 (No Credits Accepted)"

        **Result**: Keeps certain sessions premium/special, preserves credit value for regular training
      </Tab>
      <Tab title="Free">
        **Configuration:**

        - Set pricing model to **"Free"**
        - No payment or credits required

        **When to use**:

        - Lead magnets
        - Courtesy makeup sessions
        - Community events
        - Intro offers

        **Example**: "Free 30-Min Assessment"

        **Result**: No barrier to entry, acquisition tool
      </Tab>
    </Tabs>
  </Step>
  <Step title="Set credit amount (if using credits)">
    **How many credits does this session cost?**

    Most common: `1` credit = 1 session

    **Multi-credit scenarios**:

    | Session Type       | Duration | Credit Cost      | Rationale                             |
    | ------------------ | -------- | ---------------- | ------------------------------------- |
    | Standard Session   | 60 min   | 1 credit         | Baseline                              |
    | Quick Check-In     | 30 min   | 0.5 credits      | Half time = half cost                 |
    | Intensive Training | 90 min   | 1.5 or 2 credits | Longer session                        |
    | Premium Service    | 60 min   | 2 credits        | Higher value (video analysis, etc.)   |
    | Small Group Class  | 60 min   | 1 credit         | Same as private (or 0.5 for discount) |

    <Tip>
      **Simplicity rule**: Stick to whole numbers (1, 2, 3) unless you have clear reason for fractional credits (0.5, 1.5). Whole numbers are less confusing for athletes.
    </Tip>
  </Step>
  <Step title="Save scheduler configuration">
    Click **Save** or **Update Scheduler**.

    Your scheduler now accepts credits!

    <Check>
      **Success**: Athletes with credits can now book this scheduler using their credit balance. Credits will be automatically deducted at booking.
    </Check>
  </Step>
  <Step title="Configure additional schedulers">
    Repeat steps 2-6 for each scheduler you want to make credit-enabled.

    **Recommended configuration strategy**:

    **For simple credit system** (all schedulers same cost):

    - All schedulers: 1 credit each
    - Consistency = clarity for athletes

    **For tiered credit system** (different values):

    - Standard sessions: 1 credit
    - Premium services: 2 credits
    - Quick sessions: 0.5 credits
    - Assessment: Payment-only (no credits)

    **For membership models** (scheduler restrictions):

    - Basic Member schedulers: 1 credit each
    - Premium Member schedulers: 1 credit each (different scheduler list)
    - Configure via product's "Redeemable Schedulers" setting
  </Step>
</Steps>

---

## Verifying the product-scheduler linkage

The two-way connection must be properly configured:

### Verification Checklist

<Steps>
  <Step title="Check product configuration">
    Open your credit-issuing product in Product Builder.

    **Verify**:

    - ✅ "Add credits once purchased" is enabled
    - ✅ Credit amount is set (e.g., 10 credits)
    - ✅ "Redeemable Schedulers" shows correct scheduler list
      - Blank = Master Credits (any scheduler)
      - Specific list = Only those schedulers

    **If missing**: Athletes can buy credits but won't see them as usable for intended schedulers
  </Step>
  <Step title="Check scheduler configuration">
    Open your scheduler in Scheduler Settings.

    **Verify**:

    - ✅ Payment section includes "Credits" as payment option
    - ✅ Credit amount is set (e.g., 1 credit)
    - ✅ If hybrid: Both credit and payment amounts configured

    **If missing**: Athletes with credits won't be able to use them for this scheduler
  </Step>
  <Step title="Test the connection">
    **Quick test**:

    1. Create test athlete
    2. Manually grant them credits (from your product)
    3. Have test athlete attempt to book scheduler
    4. Verify credit option appears in booking flow

    **What you should see**:

    - If Master Credits: "Use 1 Credit" option on ANY credit-enabled scheduler
    - If Scheduler-Specific: "Use 1 Credit" option ONLY on linked schedulers

    **Red flag**: If credit option doesn't appear, linkage is broken—review both product and scheduler settings
  </Step>
</Steps>
