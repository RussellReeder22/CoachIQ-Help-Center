...
icon: "calendar"
keywords: ["calendar", "calendar view", "schedule view", "see bookings", "daily view", "weekly view", "list view", "month view", "coach on calendar", "resources on calendar", "quick add coach", "quick add resource", "sticky header", "mobile calendar"]
---
...
<Frame>
  ![CoachIQ calendar overview showing monthly schedule with booked sessions](/images/calendar-view.png)
</Frame>
...
## Calendar in the native admin app

The current native app includes **Day**, **Week**, **Month**, and **List** views. Tap a session to see its time, duration, location, coaches, and participants, matching the desktop session details.

Use the session detail controls to open its editors, add participants, prepare a message, or update attendance. Attendance changes remain saved when you reopen the session. See [Checking in athletes](/scheduling/checking-in-athletes).

## Saved views and open availability

For a screenshot walkthrough, see [Saved calendar views and open availability](/scheduling/saved-calendar-views).

Use the view chips above the calendar to switch between saved configurations. **All sessions** returns to your normal schedule.

1. Open **Show** and choose **Open availability only** to hide booked sessions and show openings.
2. Choose the opening types you need: **Coaches**, **Resources**, **Locations**, or **Other schedulers**. Use the schedule filters and date controls to narrow the results.
3. Choose **+ Save view**, give the view a name, and save it. Use **Manage views** to manage your saved views.

Openings appear as empty session cards without participant avatars. Resource openings represent unrented resources. They are availability, not confirmed bookings or reservations.

For a large schedule, start with a week or a smaller selection of schedulers. If the calendar asks for a shorter range or fewer schedulers, narrow the request and retry. Use the timezone control to review times in the appropriate zone.
...
CoachIQ offers four distinct views, each optimized for different tasks. All four were rebuilt for large schedules: the date bar and time column stay pinned while you scroll in Week and Day view, session cards carry the same details in every view, and the calendar works down to phone and tablet widths.

Every session card, in every view, shows the same details: session name, time, participant initials, the **coach** running it, and any attached **resources** (the cage, court, or room), so you can see who is running a session and where without clicking into it.
...
- Hour-by-hour grid for the selected day, with the time column pinned on the left as you scroll
- All sessions as fixed-width cards; sessions at the same time sit side by side and the grid scrolls horizontally when they do not fit
- Participant initials in each session
- Session names, coach, and resources
...
<Frame>
  ![CoachIQ calendar day view showing hourly session grid with participant initials](/images/calendar-day-view.png)
</Frame>
...
- 7-day grid (Sunday - Saturday) with the date bar pinned at the top and the time column pinned on the left while you scroll in either direction
- All sessions across the week as cards; columns are fixed-width and scroll horizontally so long session names and several participants still fit
- Participant initials, coach, and resources per session
- When a time cell holds more sessions than fit, hover it to reveal the rest
...
<Frame>
  ![CoachIQ calendar week view displaying seven-day session grid with color-coded bookings](/images/calendar-week-view.png)
</Frame>
...
- Full-month calendar grid, with the trailing and leading days of the neighbouring months shown faintly for context
- Session count circle on each day
- Session cards with participant initials, coach, and resources
...
<Frame>
  ![CoachIQ calendar month view showing session counts and booking density per day](/images/calendar-view.png)
</Frame>
...
- Chronological list of all bookings, grouped under a header row for each day
- Three filter tabs: **Upcoming**, **Canceled**, **Past**
- A fixed table header, and the current day's header pins to the top as you scroll (Monday pins, then Tuesday takes over)
- Today highlighted
- Detailed information per session:
  - Athletes (stacked avatars for group sessions)
  - Time (the date is in the day header)
  - Duration
  - Session/scheduler name, coach, and resources

The List view works down to phone and tablet widths, so it is the view to use on a mobile browser.
...
<Frame>
  ![CoachIQ calendar list view showing upcoming bookings with athlete names and details](/images/list-view-upcoming.png)
</Frame>
...
Each session card names the coach assigned to that occurrence and lists its attached resources, in Month, Week, and Day view alike. Use them to answer "who has the 4 pm?" and "is Cage 2 free at 5?" without opening the session. With a scheduler on **Use first available**, the card shows the specific resource that booking reserved. Details on how resources are assigned: [Resources](/scheduling/resources).
...
**Quick Add a coach or resource**: if the occurrence has no coach or is missing a resource, add one right here from the click view. The coach or resource is attached to this occurrence only, so a one-off cover coach or an extra cage for a busy night does not change the scheduler. To change every future occurrence, edit the scheduler's **Coaches** or **Resources** section instead.
...
<Tabs>
  ...
  <Tab title="Move / Edit / Cancel Session">
    **Function**: Change the occurrence or cancel it entirely

    **Edit options**:

    - **Move Session** (header): move the whole occurrence, with every booking, coach, and resource, to a new date and time. See [Move an entire session](/scheduling/editing-rescheduling-bookings#move-an-entire-session)
    - **Duration → Edit**: change the length of this occurrence without changing its start
    - **Location → Edit** and **Coaches → Edit**
    - Add/remove participants

    **Cancel options** (red trash icon in the header, opens **Cancel Session**):

    - Cancel the entire occurrence and every booking on it
    - Choose whether to refund credits
    - Notify participants by Announcement, SMS, or In-app message. See [Cancel an entire session](/scheduling/canceling-bookings#cancel-an-entire-session-from-the-calendar)
...
<AccordionGroup>
  ...
  <Accordion title="What do the numbers on month view mean?">
    ...
    **Additional info**: Below the number, the day's session cards show participant initials, coach, and resources
  ...
  <Accordion title="Can I use the calendar on my phone?">
    Yes. The calendar, including the **List** view with its pinned day headers, works at phone and tablet widths in a mobile browser. For the fastest check-ins and day-of changes on a phone, use List view for today and open sessions from there.
  </Accordion>
</AccordionGroup>
...
**More sessions than fit in a Week cell**: hover the cell to reveal the rest, or switch to Day view for that date
...
<CardGroup cols={2}>
  <Card title="Managing Conflicts" icon="triangle-exclamation" href="/scheduling/managing-availability-for-multiple-coaches">
...
<CardGroup cols={3}>
  ...
  <Card title="Multi-Coach Management" icon="users" href="/scheduling/managing-availability-for-multiple-coaches">
    Coordinate multiple coaches on one calendar
  </Card>
  <Card title="Athlete Booking Flow" icon="mobile" href="/scheduling/book-your-first-session">
