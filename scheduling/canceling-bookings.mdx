...
icon: "times-circle"
keywords: ["cancel booking", "cancel session", "cancellation", "delete booking", "remove booking", "credit refund on cancel"]
---

<Info>
  **Quick navigation**: Schedule → Calendar → Click session → trash (**Cancel Session**) or cancel a single booking
</Info>
...
## Controlling whether athletes can cancel

You can enable or disable athlete self-cancellation on a per-scheduler basis. When disabled, only coaches and admins can cancel bookings — athletes must contact you directly.

Find this in your scheduler under **Settings → Booking Rules → Allow athletes to cancel**.

You can also control whether credits are refunded on cancellation with the **Refund credits used on cancelation** toggle in the same section. See [Scheduler Settings: Limits and Permissions](/scheduling/scheduler-settings-limits-and-permissions) for all Booking Rules options.
...
<Steps>
  ...
  <Step title="Open booking details">
    ...
    <Frame>
      ![Session detail modal showing booking participants and actions](/images/session-detail-modal.png)
    </Frame>

    You'll see:

    - Athlete name and participants list
    - Session time/date
    - Scheduler type
    - Action buttons including **Send Message to All**
  </Step>
  ...
  <Step title="Confirm cancellation">
    ...
    <Frame>
      ![CoachIQ cancellation confirmation dialog with credit refund option](/images/credit-refund.png)
    </Frame>
...
## Cancel an entire session from the calendar

Use this when you need to close the whole occurrence (every participant) in one step — for example coach illness or a facility conflict — instead of canceling each booking one by one.

Where to find this: **Schedule → Calendar → click the session**

### Open Cancel Session

On the session click view, the header shows **Move Session** and a red trash icon. Click the trash icon.

That opens **Cancel Session**.

### What Cancel Session does

The dialog warns that this closes the occurrence so it cannot be booked again and cancels every current participant booking. It also shows how many participants will be canceled.

### Notify participants (optional)

**Notify participants** is on by default (recipient count shown). Choose channels:

- **Announcement**
- **SMS**
- **In-app message** (pre-checked)

### Message

The **Message** field is prefilled with the canceled session name, day, and time. Edit it to add a personal note.

### Keep Session or Cancel Session

- **Keep Session** — close without canceling
- **Cancel Session** — confirm and cancel the occurrence

Credit refund prompts can appear when participants used credits. Free sessions may not show credit controls.

---

## Canceling many separate sessions

To cancel many different calendar days (for example a full week of unrelated sessions), open each session and use **Cancel Session**, or cancel individual bookings as above. Filter the calendar to the date range you need to move faster.
...
<CardGroup cols={2}>
  ...
  <Card title="Credit System Guide" icon="coins" href="/payments/credits/credit-system-overview">
...
<CardGroup cols={3}>
  <Card title="Booking Types" icon="info-circle" href="/scheduling/understanding-booking-types">
...
Need to move a session instead of canceling? See [Editing & Rescheduling Bookings](/scheduling/editing-rescheduling-bookings).
