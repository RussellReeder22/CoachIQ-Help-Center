...
<Steps>
  <Step title="Find the booking">
    **Three ways to access:**
    ...
    3. **From Bookings List**: Schedule → Schedulers → Click booking
...
<AccordionGroup>
  ...
  <Accordion title="Athlete cancels late (violates policy)">
    ...
    **Example strict message**: "Hi Sarah, I see you canceled today's session. Per our 24-hour policy, the credit has been forfeited. Let's make sure to cancel earlier next time!"
  ...
  <Accordion title="Athlete no-shows">
    ...
    **Coach action required**:
    ...
    1. Mark session as "no-show" or canceled
    2. Message athlete to check in
    3. Enforce no-show policy
    4. Consider pattern (multiple no-shows = conversation needed)

    **Example message**: "Hey Chris, I noticed you didn't make it to today's 5pm session. Everything okay? Per our policy, the credit has been used. Let me know if you want to reschedule!"

    <Warning>
      **Chronic no-shows**: After 2-3 no-shows, consider requiring prepayment or deposits for future bookings.
    </Warning>
  </Accordion>
  <Accordion title="Canceling group session with participants">
    ...
    **What happens:**
    ...
    - ✅ All 8 bookings canceled simultaneously
    - ✅ All athletes notified at once
...
| Paid session canceled (proper notice) | ✅ Yes             | Refund to original payment method                        |
...
<CardGroup cols={2}>
  <Card title="Managing Waitlists" icon="list" href="/scheduling/managing-waitlists">
    When sessions are full, learn how to handle waitlists for cancellations
  </Card>
...
<CardGroup cols={3}>
  ...
  <Card title="Manual Booking" icon="user-plus" href="/scheduling/manually-booking-athletes">
    How to create bookings that you might later cancel
  </Card>
  ...
  <Card title="Athlete Booking Flow" icon="mobile" href="/scheduling/athlete-booking-flow">
    What athletes see when they cancel their own bookings
  </Card>
  <Card title="Resolving Conflicts" icon="exclamation-triangle" href="/scheduling/resolving-conflicts">
    Handle double-bookings and scheduling conflicts
  </Card>
