...
icon: "plus-circle"
keywords: ["create scheduler", "new scheduler", "scheduler setup", "first scheduler", "bookable session", "session template"]
---
...
<Steps>
  ...
  <Step title="Start creating new scheduler">
    ...
    <Frame>
      <img
        src="/images/create-new-scheduler.png"
...
<Steps>
  <Step title="Enter scheduler name">
    ...
    **Avoid in name**:
    ...
    <Frame>
      ![CoachIQ scheduler builder name input field](/images/scheduler-name.png)
    </Frame>
    <Tip>
...
<Steps>
  <Step title="Locate the Availability section">
    ...
    <Frame>
      ![CoachIQ scheduler availability section with dropdown to select availability template](/images/set-scheduler-availablity.png)
    </Frame>
  </Step>
  ...
  <Step title="Set Start and End Dates (optional)">
    ...
    **Good to use if you're:**
    ...
    <Frame>
      ![CoachIQ scheduler start and end date configuration for availability](/images/current-avaliabilty.png)
    </Frame>
  </Step>
  <Step title="Add an Additional Availability (optional)">
    If you need to attach another availability to your scheduler, click **Add Additional Availability** and select one.
    ...
    **Good to use if you're:**
    ...
    <Frame>
      ![CoachIQ additional availability tab for attaching a second schedule](/images/additional-availability.png)
    </Frame>
  </Step>
  <Step title="Adjust individual dates (optional)">
    Below the availability pickers, **Specific-date settings** lets you give one configured date its own duration or resources, and each date can carry its own venue. You will only need this for sessions that vary by date, such as a team that practices indoors for 90 minutes on Tuesdays and on the field for 150 minutes on Saturdays. Details: [A different duration on one date](/scheduling/advanced-scheduler-settings#a-different-duration-on-one-date), [Different resources on different dates](/scheduling/resources#different-resources-on-different-dates), [A different venue for each date](/scheduling/setup-locations#a-different-venue-for-each-date).
  </Step>
</Steps>

<Warning>
  **Availability is required to save.** If you click **Create** or **Update** without an availability selected, the builder shows an error and does not save, instead of creating a scheduler that can never be booked. Pick an availability template (or create one) and save again. The builder also previews what athletes will see once availability is connected, so you can confirm slots appear before you publish.
</Warning>

<Tip>
  **Preview a specific date before sharing.** Select a row in the connected specific-dates table to see that day's bookable times. **Open availability** opens the matching date in a new tab without losing unsaved scheduler work. See [Preview a specific date](/scheduling/advanced-scheduler-settings#preview-a-specific-date).
</Tip>
...
<Tabs>
    ...
    **How to set up**:
    ...
    <Steps>
      <Step title="Select payment type">
        In the **Payment** section, set the toggle to **Paywalled**, then under **How they pay** choose **Price**
      </Step>
      <Step title="Connect or create product">
        ...
        <Frame>
          ![CoachIQ product selection dropdown for direct payment scheduler](/images/select-payment-scheduler.png)
        </Frame>
      </Step>
    </Steps>
    ...
    **How to set up**:
    ...
    <Steps>
      <Step title="Enable credit payment">
        In the **Payment** section, set the toggle to **Paywalled**, then under **How they pay** choose **Credit**
      </Step>
      <Step title="Set credit cost">
        ...
        <Frame>
          ![Credit cost input field](/images/credit-payment-scheduler.png)
        </Frame>
      </Step>
  ...
  <Tab title="Hybrid (Direct Payment OR Credits)">
    ...
    **How to set up**:
    ...
    <Frame>
      ![CoachIQ hybrid payment setup showing both credit and direct payment options](/images/credit-and-payment-options.png)
    </Frame>
    **Result**: Athletes can either pay the direct price or redeem a credit—their choice.

    <Frame>
      ![Athlete booking view showing option to pay with credit or direct payment](/images/session-payment-flexibility.png)
    </Frame>
    <Note>
...
<AccordionGroup>
  ...
  <Accordion title="Rescheduling">
    **Settings**: Can athletes reschedule their own bookings?

    **Allow athletes to reschedule** — When on, athletes can move their booking to a different available time slot from the Athlete Portal or app without contacting you. No additional payment is required; the original credit or payment carries over to the new time.

    **Block rescheduling within cancellation window** — When on, athletes cannot reschedule once they are inside your cancellation notice window. This closes the loophole where an athlete avoids a credit forfeit by rescheduling instead of canceling. The window used is the same one set in your Cancellation Policy (e.g., if you require 24-hour notice, rescheduling is also blocked within that same 24 hours).

    **Recommended**: Enable both for paid schedulers — allowing rescheduling reduces cancellations, while blocking rescheduling inside the cancellation window prevents athletes from using it to bypass your policy.
  </Accordion>
...
<Steps>
  <Step title="Locate buffer time settings">
    ...
    <Frame>
      ![CoachIQ buffer time settings for pre-session and post-session breaks](/images/buffer.png)
    </Frame>
  </Step>
...
<Steps>
  <Step title="Enable reminders">
    ...
    <Frame>
      ![CoachIQ reminder settings for automated session notifications](/images/Reminders-UI.png)
    </Frame>
  </Step>
...
<Steps>
  <Step title="Locate location section">
    ...
    <Frame>
      ![CoachIQ scheduler location settings with venue selection](/images/location-scheduler-ui.png)
    </Frame>
  </Step>
...
<Steps>
  <Step title="Find connected coaches section">
    ...
    <Frame>
      ![CoachIQ connected coaches section for assigning coaches to a scheduler](/images/connected-coaches-ui.png)
    </Frame>
  </Step>
  ...
  <Step title="Set same-time booking permission">
    In the **Connected Coaches** section you'll find this checkbox: "Check this box to allow bookings for this session even if the connected coaches are already scheduled for other sessions at the same time."

    - **Unchecked** (default, recommended) - Prevents the coach from being double-booked across sessions
    - **Checked** - Allows the coach to be booked for this session even when already scheduled elsewhere at the same time (use for group sessions where one coach leads multiple)

    **Recommended**: Leave unchecked unless you specifically need the coach to run overlapping sessions.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Note your scheduler details">
    ...
    <Frame>
      ![CoachIQ schedulers list showing name, duration, and pricing for each scheduler](/images/schedulers-overview.png)
    </Frame>
  </Step>
</Steps>
...
<Steps>
  <Step title="Get your booking link">
    ...
    <Frame>
      ![Copy booking link button on CoachIQ schedulers list](/images/scheduler-link.png)
    </Frame>
  </Step>
  <Step title="Open booking link as athlete would">
    ...
    **What you should see**:
    ...
    <Frame>
      ![Athlete-facing scheduler booking page showing available time slots and pricing](/images/scheduler-athlete-view.png)
    </Frame>
  </Step>
  ...
  <Step title="Check your calendar">
    ...
    <Frame>
      ![Calendar showing scheduler availability and test booking](/images/calendar-view.png)
    </Frame>
    <Check>
...
<AccordionGroup>
  <Accordion title="I can't save my scheduler (save button disabled)">
    ...
    **How to fix**:
    ...
    3. Most common missing items:
       - Scheduler name
       - Duration
       - Availability (none selected — the builder shows an explicit error for this and will not save until one is connected)
       - Payment configuration incomplete
       - A specific date whose time window is shorter than the duration that applies to it (the error names the date; widen the window or shorten that date's duration)
  ...
  <Accordion title="No available slots showing when I test">
    ...
    **Possible causes**:
    ...
    1. **Connected coach already booked elsewhere** - If the coach assigned to this scheduler has sessions booked through other schedulers at those times, slots won't appear (this is double-booking prevention working correctly)
    2. **No availability set** - Forgot to add days/times
    3. **Availability in past** - End date already passed
    4. **All dates blocked** - Blackout days covering everything
    5. **Buffer time too long** - Consuming entire availability window

    **Read the message the athlete sees**: a time that is simply outside your availability now shows a plain "no availability" message. A coach-conflict message only appears when a connected coach really is booked at that time, so if an athlete reports one, check the calendar for that coach; if they report no availability, check the availability template and date range instead.

    **How to fix**:

    - **Check for coach conflicts first**: Go to Schedule → Calendar and check if the connected coach has existing sessions during the times you expect to see availability
    ...
    For detailed troubleshooting, see [How availability works](/scheduling/how-availability-works)
  </Accordion>
...
<CardGroup cols={3}>
  ...
  <Card title="Creating Availability" icon="calendar-check" href="/scheduling/create-your-availability">
  ...
  <Card title="Troubleshooting" icon="tools" href="/quick-guides/athletes/sessions-not-showing">
