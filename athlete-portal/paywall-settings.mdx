...
![CoachIQ paywalled athlete app showing purchase prompt for restricted content](/images/paywalled-athlete-app.png)
...
![CoachIQ Athlete Portal paywall page settings configuration](/images/athleteportal-paywall-page-settings.png)
...
<Note>
  **Required before starting:**
  ...
  - At least one product created in [Payments → Products](/payments/product-overview)
...
![CoachIQ Products page in the admin dashboard](/images/Products-page.png)
...
<Steps>
  ...
  <Step title="Select pages to paywall">
    ...
    - **Schedulers** - The Schedule page and the schedulers you have added to it

    ![CoachIQ page selection checkboxes for choosing which pages to paywall](/images/selectwhichpagesyouwishtopaywall.png)

    <Note>
      Paywalling **Schedulers** restricts only the schedulers listed on your portal's Schedule page. A scheduler you link from a form, a website button, or a direct link stays bookable without the paywalled product, so intake forms and evaluation links keep working for prospects. To gate one of those schedulers too, attach a product to it directly — see [Attach products to schedulers](/payments/attach-products).
    </Note>
  </Step>
  <Step title="Add qualifying products">
    ...
    ![CoachIQ product selection modal for assigning products to a paywalled page](/images/selectproducttopaywallspecificpage.png)
  </Step>
  ...
  <Step title="Update and activate">
    ...
    <Warning>
      A paywall needs an active page to sit in front of. If you turn the paywall on but no paywalled page is active, athletes are prompted to pick a page instead of seeing your content. Activate the page in **Menu & Links**, then save the paywall again.
    </Warning>
  </Step>
</Steps>
...
![Athlete view of a paywalled page with product pricing and purchase button](/images/paywalled-athlete-app.png)
...
<Note>
  The paywall only stands in front of the page itself. An athlete who reaches a scheduler through a form, a button, or a direct link (rather than the Schedule page) can book it without the paywalled product, and a program you shared directly with an athlete stays visible even when the Programs page is paywalled.
</Note>
...
![CoachIQ paywall page SEO settings with title, description, and URL slug](/images/CleanShot2025-09-26at20.20.26.png)
...
1. **No qualifying product purchased**
   - Check: [People](/people/overview) → [Athlete Name] → Products tab
   - Solution: Direct athlete to purchase page on web portal
2. **Subscription expired**
   - Check: [Payments → Subscriptions](/payments/subscriptions-dashboard) → Filter by athlete
   - Solution: Update payment method or reactivate subscription
3. **Payment failed**
   - Check: [Payments → Transactions](/payments/transactions) → Failed payments
...
6. **Paywall on, but no active page selected**
   - Symptom: athletes see a prompt to pick a page instead of the paywall or the content
   - Solution: Activate at least one paywalled page in Menu & Links, then click **Update Paywall** again
...
<AccordionGroup>
  <Accordion title="Does the Schedulers paywall block every scheduler I own?">
    No. It blocks the schedulers you added to the portal's Schedule page. Schedulers reached through a form, a website button, or a direct link are not gated by the Pages Paywall. If you want one of those gated as well, attach a product to that scheduler in its own settings.
  </Accordion>
  ...
  <Accordion title="How do I offer a free trial before the paywall?">
    Create a subscription product with a trial period in [Payments → Products](/payments/product-overview). Set the trial length (e.g., 7 days free), then add this product to your paywall.
  </Accordion>
  <Accordion title="Do paywalls work on both mobile app and web?">

    Paywalls control access on web and native apps. The available purchase flow depends on the installed app and eligible purchase. When native checkout is unavailable, use the web portal to purchase, then return to the app.
...
- [Payments → Transactions](/payments/transactions) (purchase tracking)
- [Payments → Subscriptions](/payments/subscriptions-dashboard) (active membership monitoring)
- [People](/people/overview) → Activity logs (access patterns)

## Related articles

<CardGroup cols={2}>
  <Card title="Product Overview" icon="box" href="/payments/product-overview">
    Understand how products work and where athletes can find them
  </Card>
  <Card title="Product Builder" icon="credit-card" href="/payments/product-builder">
    Create memberships, packages, and one-time products
  </Card>
  <Card title="Attach Products to Schedulers" icon="calendar-plus" href="/payments/attach-products">
    Require payment or credits when athletes book sessions
  </Card>
  <Card title="Products in Your Athlete Portal" icon="mobile" href="/athlete-portal/products">
    Display and organize products in your athlete-facing portal
  </Card>
  <Card title="Transactions" icon="credit-card" href="/payments/transactions">
    Track purchases, refunds, and payment history
  </Card>
  <Card title="Managing People" icon="users" href="/people/overview">
    View athlete profiles, products, and activity
  </Card>
